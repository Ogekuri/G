...
- Internal Call-Trace Tree:
  - `__main__::<module_guard>(...)`: module execution bridge [`src/git_alias/__main__.py`]
    - `main(...)`: CLI dispatch root that executes update-check before argument validation [`src/git_alias/core.py`]
      ...
      - `upgrade_self(...)`: self-upgrade action using `UV_TOOL_NAME` for install target [`src/git_alias/core.py`]
        - `_run_checked(...)`: spawn `uv tool install <UV_TOOL_NAME> --force --from git+https://github.com/<owner>/<repo>.git` [`src/git_alias/core.py`]
