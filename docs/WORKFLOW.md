...
- Internal Call-Trace Tree:
  - `__main__::<module_guard>(...)`: module execution bridge [`src/git_alias/__main__.py`]
      ...
      - `check_for_newer_version(...)`: optional remote release check and cache handling; executes only when `~/.cache/git-alias/check_version_idle-time.json` is absent or expired unless explicit ignore-idle-cache forcing is requested, writes 300-second idle-delay windows after success, and writes fixed 3600-second idle-delay windows after HTTP 429 or HTTP 403 API rate-limit responses [`src/git_alias/core.py`]
