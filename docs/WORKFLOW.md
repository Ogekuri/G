...
- Internal Call-Trace Tree:
  - `__main__::<module_guard>(...)`: module execution bridge [`src/git_alias/__main__.py`]
      ...
      - `check_for_newer_version(...)`: optional remote release check and cache handling; executes only when `~/.cache/git-alias/check_version_idle-time.json` is absent or expired unless explicit ignore-idle-cache forcing is requested, writes fixed 3600-second idle-delay windows after success, and writes fixed 86400-second idle-delay windows after release-check API-call errors [`src/git_alias/core.py`]
        ...
        - `_write_version_check_api_error_state(...)`: persist the canonical extended idle-time state for release-check API-call errors [`src/git_alias/core.py`]
