...
- Requirements traceability (code-to-spec evidence).
  ...
  - `REQ-024`/`REQ-032` alignment: Doxygen-style machine-readable annotations are attached to module symbols and callable nodes in `src/git_alias/core.py`, `src/git_alias/__init__.py`, `src/git_alias/__main__.py` via structured `@brief`, `@param`, `@return` tags (comment blocks immediately preceding each declaration).
