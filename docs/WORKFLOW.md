  ...
  - `__main__::<module_guard>(...)`: module execution bridge [`src/git_alias/__main__.py:10-11`]
    - `main(...)`: CLI dispatch root [`src/git_alias/core.py:2671`]
      ...
      - `COMMANDS[name](extras)`: registered alias dispatch path [`src/git_alias/core.py:2549-2614`, `src/git_alias/core.py:2716`]
        ...
        - `cmd_ls(...)`: wrapper -> `run_git_cmd(...)` -> `_to_args(...)` -> `_run_checked(...)` [`src/git_alias/core.py:2514`]
        - `cmd_lsi(...)`: wrapper -> `run_git_cmd(...)` -> `_to_args(...)` -> `_run_checked(...)` [`src/git_alias/core.py:2523`]
        - `cmd_lt(...)`: tag visibility flow -> `capture_git_output(...)` (`git tag -l`) -> `_to_args(...)` -> `_run_checked(...)`; per tag `capture_git_output(...)` (`git branch -a --contains <tag>`) -> `_run_checked(...)` -> stdout formatter [`src/git_alias/core.py:2532`]
