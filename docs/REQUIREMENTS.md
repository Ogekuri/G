...
- **REQ-024**: Ogni funzione definita in `core.py` deve includere documentazione Doxygen in formato machine-readable (`@brief`, `@param`, `@return` quando applicabile), coerente con l'implementazione corrente e allineata alle linee guida `guidelines/Document_Source_Code_in_Doxygen_Style_for_LLM-Agent.md`.
...
- **REQ-032**: Tutti i componenti del codice sorgente (`moduli`, `classi`, `funzioni/metodi`, `variabili esportate`) devono mantenere copertura documentale Doxygen completa, in lingua inglese US, con descrizioni a densità semantica alta orientate a LLM Agent e parser automatici; ogni modifica funzionale deve aggiornare contestualmente la documentazione correlata per evitare disallineamenti.
