...
# core.py | Python | 3054L | 182 symbols | 16 imports | 801 comments
...
- Param: key Input parameter consumed by `_load_config_rules`. fallback Input parameter consumed by `_load_config_rules`.
...
- fn `def __init__(self, exc: subprocess.CalledProcessError)` `priv` (L547-554)
  ...
  - Param: self Input parameter consumed by `__init__`. exc Input parameter consumed by `__init__`.
...
- Param: *popenargs Input parameter consumed by `_run_checked`. **kwargs Input parameter consumed by `_run_checked`.
...
- Param: base_args Input parameter consumed by `run_git_cmd`. extra Input parameter consumed by `run_git_cmd`. cwd Input parameter consumed by `run_git_cmd`. **kwargs Input parameter consumed by `run_git_cmd`.
...
- Param: base_args Input parameter consumed by `capture_git_output`. cwd Input parameter consumed by `capture_git_output`.
...
- Param: cmd Input parameter consumed by `run_command`. cwd Input parameter consumed by `run_command`.
...
- Param: args Input parameter consumed by `run_git_text`. cwd Input parameter consumed by `run_git_text`. check Input parameter consumed by `run_git_text`.
...
- Param: command Input parameter consumed by `run_shell`. cwd Input parameter consumed by `run_shell`.
...
- Param: branch_key Input parameter consumed by `_branch_remote_divergence`. remote Input parameter consumed by `_branch_remote_divergence`.
...
- Param: branch_key Input parameter consumed by `has_remote_branch_updates`. remote Input parameter consumed by `has_remote_branch_updates`.
...
- Param: commit_hash Input parameter consumed by `_commit_exists_in_branch`. branch_name Input parameter consumed by `_commit_exists_in_branch`.
...
- Brief: Class `TagInfo` models a typed runtime container/error boundary. Store raw tag name including `v` prefix when present. Store ISO date string used for changelog section headers. Store object hash associated with the tag reference.
...
- Param: all_tags Full list of `TagInfo` sorted by date ascending (from `list_tags_sorted_by_date`). last_minor The last minor-release `TagInfo` to anchor the search; `None` means no minor exists.
...
- Param: repo_root Input parameter consumed by `list_tags_sorted_by_date`. merged_ref Input parameter consumed by `list_tags_sorted_by_date`.
- Return: Result emitted by `list_tags_sorted_by_date` according to command contract.

### fn `def git_log_subjects(repo_root: Path, rev_range: str) -> List[str]` (L987-998)
...
- Param: repo_root Input parameter consumed by `git_log_subjects`. rev_range Input parameter consumed by `git_log_subjects`.
- Return: Result emitted by `git_log_subjects` according to command contract.

### fn `def parse_conventional_commit(message: str) -> Optional[Tuple[str, Optional[str], bool, str]]` (L1004-1016)
...
### fn `def _format_changelog_description(desc: str) -> str` `priv` (L1022-1032)
...
### fn `def categorize_commit(subject: str) -> Tuple[Optional[str], str]` (L1039-1064)
...
### fn `def _extract_release_version(subject: str) -> Optional[str]` `priv` (L1069-1079)
...
### fn `def _is_release_marker_commit(subject: str) -> bool` `priv` (L1084-1087)
...
### fn `def generate_section_for_range(repo_root: Path, title: str, date_s: str, rev_range: str, expected_version: Optional[str] = None) -> Optional[str]` (L1096-1134)
...
- Param: repo_root Input parameter consumed by `generate_section_for_range`. title Input parameter consumed by `generate_section_for_range`. date_s Input parameter consumed by `generate_section_for_range`. rev_range Input parameter consumed by `generate_section_for_range`. expected_version Input parameter consumed by `generate_section_for_range`.
- Return: Result emitted by `generate_section_for_range` according to command contract.

### fn `def _get_remote_name_for_branch(branch_name: str, repo_root: Path) -> str` `priv` (L1143-1151)
...
- Param: branch_name Local branch name whose configured remote is requested (e.g. `"master"`). repo_root Absolute path used as CWD for the git config query.
- Return: Remote name string; never empty (falls back to `"origin"`).
- Satisfies: REQ-046

### fn `def _extract_owner_repo(remote_url: str) -> Optional[Tuple[str, str]]` `priv` (L1158-1182)
...
### fn `def _canonical_origin_base(repo_root: Path) -> Optional[str]` `priv` (L1192-1205)
...
### fn `def get_origin_compare_url(base_url: Optional[str], prev_tag: Optional[str], tag: str) -> Optional[str]` (L1212-1219)
...
- Param: base_url Input parameter consumed by `get_origin_compare_url`. prev_tag Input parameter consumed by `get_origin_compare_url`. tag Input parameter consumed by `get_origin_compare_url`.
- Return: Result emitted by `get_origin_compare_url` according to command contract.

### fn `def get_release_page_url(base_url: Optional[str], tag: str) -> Optional[str]` (L1225-1230)
...
- Param: base_url Input parameter consumed by `get_release_page_url`. tag Input parameter consumed by `get_release_page_url`.
- Return: Result emitted by `get_release_page_url` according to command contract.

### fn `def build_history_section(` (L1238-1242)
...
- Param: repo_root Input parameter consumed by `build_history_section`. tags Input parameter consumed by `build_history_section`. include_unreleased Input parameter consumed by `build_history_section`. include_unreleased_link Input parameter consumed by `build_history_section`.
- Return: Result emitted by `build_history_section` according to command contract.

### fn `def generate_changelog_document(repo_root: Path, include_patch: bool, disable_history: bool = False) -> str` (L1286-1347)
...
- Param: repo_root Absolute path to the repository root used as CWD for all git commands. include_patch When `True`, prepend the latest patch release section to the document. disable_history When `True`, omit `# History` section from output.
...
### class `class VersionRuleContext` `@dataclass(frozen=True)` (L1358-1365)
- Brief: Execute `_collect_version_files` runtime logic for Git-Alias CLI. Store resolved per-rule context for version discovery and matching.
- Details: Executes `_collect_version_files` using deterministic CLI control-flow and explicit error propagation. Encapsulates immutable artifacts required across detect/update/verify phases:
compiled regex, resolved file list, and pre-rendered relative-path map for stable debug output.
- Param: root Input parameter consumed by `_collect_version_files`. pattern Input parameter consumed by `_collect_version_files`.
...
### fn `def _normalize_version_rule_pattern(pattern: str) -> str` `priv` (L1371-1382)
...
### fn `def _build_version_file_inventory(root: Path) -> List[Tuple[Path, str]]` `priv` (L1388-1409)
...
### fn `def _collect_version_files(root, pattern, *, inventory=None)` `priv` (L1417-1434)
...
Uses precomputed inventory when provided to avoid repeated repository traversals. Apply pathspec matcher to preserve configured GitIgnore-like semantics.
- Param: root Input parameter consumed by `_collect_version_files`. pattern Input parameter consumed by `_collect_version_files`. inventory Optional precomputed `(path, normalized_relative_path)` list.
- Return: Result emitted by `_collect_version_files` according to command contract.

### fn `def _is_version_path_excluded(relative_path: str) -> bool` `priv` (L1439-1442)
...
### fn `def _iter_versions_in_text(text, compiled_regexes)` `priv` (L1448-1459)
...
- Param: text Input parameter consumed by `_iter_versions_in_text`. compiled_regexes Input parameter consumed by `_iter_versions_in_text`.
- Return: Result emitted by `_iter_versions_in_text` according to command contract.

### fn `def _read_version_file_text(file_path: Path, text_cache: Optional[Dict[Path, str]] = None) -> Optional[str]` `priv` (L1466-1480)
...
- Param: file_path Absolute path of the file to read. text_cache Optional mutable cache keyed by `Path` to avoid duplicate reads across phases.
- Return: File text payload or `None` when file cannot be read.

### fn `def _prepare_version_rule_contexts(` `priv` (L1489-1490)
...
- Param: root Repository root path used for relative-path rendering. rules Sequence of `(pattern, regex)` tuples. inventory Optional precomputed inventory to avoid repeated filesystem traversal.
...
### fn `def _determine_canonical_version(` `priv` (L1532-1539)
...
- Param: root Input parameter consumed by `_determine_canonical_version`. rules Input parameter consumed by `_determine_canonical_version`. verbose Input parameter consumed by `_determine_canonical_version`. debug Input parameter consumed by `_determine_canonical_version`. contexts Optional precomputed `VersionRuleContext` list for reuse across phases. text_cache Optional mutable cache keyed by file path to avoid duplicate reads.
- Return: Result emitted by `_determine_canonical_version` according to command contract.

### fn `def _parse_semver_tuple(text: str) -> Optional[Tuple[int, int, int]]` `priv` (L1584-1590)
...
### fn `def _replace_versions_in_text(text, compiled_regex, replacement)` `priv` (L1597-1612)
...
- Param: text Input parameter consumed by `_replace_versions_in_text`. compiled_regex Input parameter consumed by `_replace_versions_in_text`. replacement Input parameter consumed by `_replace_versions_in_text`.
- Return: Result emitted by `_replace_versions_in_text` according to command contract.

### fn `def _current_branch_name()` `priv` (L1616-1628)
...
### fn `def _ref_exists(ref_name)` `priv` (L1633-1642)
...
### fn `def _local_branch_exists(branch_name)` `priv` (L1647-1650)
...
### fn `def _remote_branch_exists(branch_name)` `priv` (L1655-1658)
...
### fn `def _ensure_release_prerequisites()` `priv` (L1662-1689)
...
### fn `def _bump_semver_version(current_version, level)` `priv` (L1695-1713)
...
- Param: current_version Input parameter consumed by `_bump_semver_version`. level Input parameter consumed by `_bump_semver_version`.
- Return: Result emitted by `_bump_semver_version` according to command contract.

### fn `def _run_release_step(level, step_name, action)` `priv` (L1720-1740)
...
- Param: level Input parameter consumed by `_run_release_step`. step_name Input parameter consumed by `_run_release_step`. action Input parameter consumed by `_run_release_step`.
- Return: Result emitted by `_run_release_step` according to command contract.

### fn `def _create_release_commit_for_flow(target_version)` `priv` (L1745-1750)
...
### fn `def _push_branch_with_tags(branch_name)` `priv` (L1756-1760)
...
### fn `def _execute_release_flow(level, changelog_args=None)` `priv` (L1776-1823)
...
with `--tags`.
- Param: level Release level string: `"major"`, `"minor"`, or `"patch"`. changelog_args Optional list of extra changelog flags forwarded alongside `--force-write`.
- Return: None; raises `ReleaseError` or `VersionDetectionError` on failure.
- Satisfies: REQ-026, REQ-045

### fn `def _execute_backup_flow()` `priv` (L1831-1846)
...
### fn `def _run_release_command(level, changelog_args=None)` `priv` (L1852-1867)
...
- Param: level Input parameter consumed by `_run_release_command`. changelog_args Input parameter consumed by `_run_release_command`.
- Return: Result emitted by `_run_release_command` according to command contract.

### fn `def _run_backup_command()` `priv` (L1872-1879)
...
### fn `def _run_reset_with_help(base_args, extra)` `priv` (L1885-1892)
...
- Param: base_args Input parameter consumed by `_run_reset_with_help`. extra Input parameter consumed by `_run_reset_with_help`.
- Return: Result emitted by `_run_reset_with_help` according to command contract.

### fn `def _reject_extra_arguments(extra, alias)` `priv` (L1898-1904)
...
- Param: extra Input parameter consumed by `_reject_extra_arguments`. alias Input parameter consumed by `_reject_extra_arguments`.
- Return: Result emitted by `_reject_extra_arguments` according to command contract.

### fn `def _parse_release_flags(extra, alias)` `priv` (L1910-1928)
...
- Param: extra Input parameter consumed by `_parse_release_flags`. alias Input parameter consumed by `_parse_release_flags`.
- Return: Result emitted by `_parse_release_flags` according to command contract.

### fn `def _prepare_commit_message(extra, alias)` `priv` (L1934-1944)
...
- Param: extra Input parameter consumed by `_prepare_commit_message`. alias Input parameter consumed by `_prepare_commit_message`.
- Return: Result emitted by `_prepare_commit_message` according to command contract.

### fn `def _build_conventional_message(kind: str, extra, alias: str) -> str` `priv` (L1951-1965)
...
- Param: kind Input parameter consumed by `_build_conventional_message`. extra Input parameter consumed by `_build_conventional_message`. alias Input parameter consumed by `_build_conventional_message`.
- Return: Result emitted by `_build_conventional_message` according to command contract.

### fn `def _run_conventional_commit(kind: str, alias: str, extra)` `priv` (L1972-1977)
...
- Param: kind Input parameter consumed by `_run_conventional_commit`. alias Input parameter consumed by `_run_conventional_commit`. extra Input parameter consumed by `_run_conventional_commit`.
- Return: Result emitted by `_run_conventional_commit` according to command contract.

### fn `def _execute_commit(message, alias, allow_amend=True)` `priv` (L1984-2013)
...
- Param: message Input parameter consumed by `_execute_commit`. alias Input parameter consumed by `_execute_commit`. allow_amend Input parameter consumed by `_execute_commit`.
- Return: Result emitted by `_execute_commit` according to command contract.

### fn `def upgrade_self()` (L2017-2030)
...
### fn `def cmd_aa(extra)` (L2042-2049)
...
### fn `def cmd_ra(extra)` (L2054-2077)
...
### fn `def cmd_ar(extra)` (L2082-2096)
...
### fn `def cmd_br(extra)` (L2101-2104)
...
### fn `def cmd_bd(extra)` (L2109-2112)
...
### fn `def cmd_ck(extra)` (L2117-2120)
...
### fn `def _ensure_commit_ready(alias)` `priv` (L2125-2138)
...
### fn `def cmd_cm(extra)` (L2143-2148)
...
### fn `def cmd_wip(extra)` (L2153-2165)
...
### fn `def cmd_release(extra)` (L2170-2192)
...
### fn `def cmd_new(extra)` (L2197-2200)
...
### fn `def cmd_refactor(extra)` (L2205-2208)
...
### fn `def cmd_fix(extra)` (L2213-2216)
...
### fn `def cmd_change(extra)` (L2221-2224)
...
### fn `def cmd_implement(extra)` (L2229-2232)
...
### fn `def cmd_docs(extra)` (L2237-2240)
...
### fn `def cmd_style(extra)` (L2245-2248)
...
### fn `def cmd_revert(extra)` (L2253-2256)
...
### fn `def cmd_misc(extra)` (L2261-2264)
...
### fn `def cmd_cover(extra)` (L2269-2272)
...
### fn `def cmd_co(extra)` (L2277-2280)
...
### fn `def cmd_dcc(extra)` (L2297-2300)
...
### fn `def cmd_dccc(extra)` (L2305-2308)
...
### fn `def cmd_de(extra)` (L2313-2316)
...
### fn `def cmd_di(extra)` (L2321-2324)
...
### fn `def cmd_diyou(extra)` (L2329-2332)
...
### fn `def cmd_dime(extra)` (L2337-2340)
...
### fn `def cmd_dwc(extra)` (L2345-2348)
...
### fn `def cmd_dwcc(extra)` (L2353-2356)
...
### fn `def cmd_ed(extra)` (L2361-2370)
...
### fn `def cmd_fe(extra)` (L2375-2378)
...
### fn `def cmd_feall(extra)` (L2383-2386)
...
### fn `def cmd_gp(extra)` (L2391-2394)
...
### fn `def cmd_gr(extra)` (L2399-2402)
...
### fn `def cmd_str(extra)` (L2407-2436)
- Brief: Execute `cmd_str` runtime logic for Git-Alias CLI.
- Details: Executes `cmd_str` using deterministic CLI control-flow and explicit error propagation. Query git remotes with transport metadata. Deduplicate remote names from `git remote -v` rows. Print normalized remote name inventory. Print detailed status for each unique remote.
- Param: extra Input parameter consumed by `cmd_str`.
- Return: Result emitted by `cmd_str` according to command contract.

### fn `def cmd_lb(extra)` (L2441-2444)
...
### fn `def cmd_lg(extra)` (L2449-2462)
...
### fn `def cmd_lh(extra)` (L2467-2470)
...
### fn `def cmd_ll(extra)` (L2475-2487)
...
### fn `def cmd_lm(extra)` (L2492-2495)
...
### fn `def cmd_lt(extra)` (L2501-2520)
...
### fn `def cmd_me(extra)` (L2525-2528)
...
### fn `def cmd_pl(extra)` (L2533-2536)
...
### fn `def cmd_pt(extra)` (L2541-2544)
...
### fn `def cmd_pu(extra)` (L2549-2552)
...
### fn `def cmd_rf(extra)` (L2557-2560)
...
### fn `def cmd_rmtg(extra)` (L2565-2575)
...
### fn `def cmd_rmloc(extra)` (L2580-2583)
...
### fn `def cmd_rmstg(extra)` (L2588-2591)
...
### fn `def cmd_rmunt(extra)` (L2596-2599)
...
### fn `def cmd_rs(extra)` (L2604-2607)
...
### fn `def cmd_rssft(extra)` (L2612-2615)
...
### fn `def cmd_rsmix(extra)` (L2620-2623)
...
### fn `def cmd_rshrd(extra)` (L2628-2631)
...
### fn `def cmd_rsmrg(extra)` (L2636-2639)
...
### fn `def cmd_rskep(extra)` (L2644-2647)
...
### fn `def cmd_st(extra)` (L2652-2655)
...
### fn `def cmd_tg(extra)` (L2660-2663)
...
### fn `def cmd_unstg(extra)` (L2668-2671)
...
### fn `def cmd_ver(extra)` (L2676-2702)
...
### fn `def cmd_chver(extra)` (L2707-2779)
...
### fn `def cmd_major(extra)` (L2788-2792)
...
### fn `def cmd_minor(extra)` (L2801-2805)
...
### fn `def cmd_patch(extra)` (L2814-2818)
...
### fn `def cmd_backup(extra)` (L2826-2836)
...
### fn `def cmd_changelog(extra)` (L2846-2878)
...
### fn `def print_command_help(name, width=None)` (L2954-2960)
...
- Param: name Input parameter consumed by `print_command_help`. width Input parameter consumed by `print_command_help`.
- Return: Result emitted by `print_command_help` according to command contract.

### fn `def print_all_help()` (L2964-2998)
...
### fn `def main(argv=None, *, check_updates: bool = True)` (L3004-3054)
...
- Param: argv Input parameter consumed by `main`. check_updates Input parameter consumed by `main`.
...
|`git_log_subjects`|fn|pub|987-998|def git_log_subjects(repo_root: Path, rev_range: str) -> ...|
|`parse_conventional_commit`|fn|pub|1004-1016|def parse_conventional_commit(message: str) -> Optional[T...|
|`_format_changelog_description`|fn|priv|1022-1032|def _format_changelog_description(desc: str) -> str|
|`categorize_commit`|fn|pub|1039-1064|def categorize_commit(subject: str) -> Tuple[Optional[str...|
|`_extract_release_version`|fn|priv|1069-1079|def _extract_release_version(subject: str) -> Optional[str]|
|`_is_release_marker_commit`|fn|priv|1084-1087|def _is_release_marker_commit(subject: str) -> bool|
|`generate_section_for_range`|fn|pub|1096-1134|def generate_section_for_range(repo_root: Path, title: st...|
|`_get_remote_name_for_branch`|fn|priv|1143-1151|def _get_remote_name_for_branch(branch_name: str, repo_ro...|
|`_extract_owner_repo`|fn|priv|1158-1182|def _extract_owner_repo(remote_url: str) -> Optional[Tupl...|
|`_canonical_origin_base`|fn|priv|1192-1205|def _canonical_origin_base(repo_root: Path) -> Optional[str]|
|`get_origin_compare_url`|fn|pub|1212-1219|def get_origin_compare_url(base_url: Optional[str], prev_...|
|`get_release_page_url`|fn|pub|1225-1230|def get_release_page_url(base_url: Optional[str], tag: st...|
|`build_history_section`|fn|pub|1238-1242|def build_history_section(|
|`generate_changelog_document`|fn|pub|1286-1347|def generate_changelog_document(repo_root: Path, include_...|
|`VersionRuleContext`|class|pub|1358-1365|class VersionRuleContext|
|`_normalize_version_rule_pattern`|fn|priv|1371-1382|def _normalize_version_rule_pattern(pattern: str) -> str|
|`_build_version_file_inventory`|fn|priv|1388-1409|def _build_version_file_inventory(root: Path) -> List[Tup...|
|`_collect_version_files`|fn|priv|1417-1434|def _collect_version_files(root, pattern, *, inventory=None)|
|`_is_version_path_excluded`|fn|priv|1439-1442|def _is_version_path_excluded(relative_path: str) -> bool|
|`_iter_versions_in_text`|fn|priv|1448-1459|def _iter_versions_in_text(text, compiled_regexes)|
|`_read_version_file_text`|fn|priv|1466-1480|def _read_version_file_text(file_path: Path, text_cache: ...|
|`_prepare_version_rule_contexts`|fn|priv|1489-1490|def _prepare_version_rule_contexts(|
|`_determine_canonical_version`|fn|priv|1532-1539|def _determine_canonical_version(|
|`_parse_semver_tuple`|fn|priv|1584-1590|def _parse_semver_tuple(text: str) -> Optional[Tuple[int,...|
|`_replace_versions_in_text`|fn|priv|1597-1612|def _replace_versions_in_text(text, compiled_regex, repla...|
|`_current_branch_name`|fn|priv|1616-1628|def _current_branch_name()|
|`_ref_exists`|fn|priv|1633-1642|def _ref_exists(ref_name)|
|`_local_branch_exists`|fn|priv|1647-1650|def _local_branch_exists(branch_name)|
|`_remote_branch_exists`|fn|priv|1655-1658|def _remote_branch_exists(branch_name)|
|`_ensure_release_prerequisites`|fn|priv|1662-1689|def _ensure_release_prerequisites()|
|`_bump_semver_version`|fn|priv|1695-1713|def _bump_semver_version(current_version, level)|
|`_run_release_step`|fn|priv|1720-1740|def _run_release_step(level, step_name, action)|
|`_create_release_commit_for_flow`|fn|priv|1745-1750|def _create_release_commit_for_flow(target_version)|
|`_push_branch_with_tags`|fn|priv|1756-1760|def _push_branch_with_tags(branch_name)|
|`_execute_release_flow`|fn|priv|1776-1823|def _execute_release_flow(level, changelog_args=None)|
|`_execute_backup_flow`|fn|priv|1831-1846|def _execute_backup_flow()|
|`_run_release_command`|fn|priv|1852-1867|def _run_release_command(level, changelog_args=None)|
|`_run_backup_command`|fn|priv|1872-1879|def _run_backup_command()|
|`_run_reset_with_help`|fn|priv|1885-1892|def _run_reset_with_help(base_args, extra)|
|`_reject_extra_arguments`|fn|priv|1898-1904|def _reject_extra_arguments(extra, alias)|
|`_parse_release_flags`|fn|priv|1910-1928|def _parse_release_flags(extra, alias)|
|`_prepare_commit_message`|fn|priv|1934-1944|def _prepare_commit_message(extra, alias)|
|`_build_conventional_message`|fn|priv|1951-1965|def _build_conventional_message(kind: str, extra, alias: ...|
|`_run_conventional_commit`|fn|priv|1972-1977|def _run_conventional_commit(kind: str, alias: str, extra)|
|`_execute_commit`|fn|priv|1984-2013|def _execute_commit(message, alias, allow_amend=True)|
|`upgrade_self`|fn|pub|2017-2030|def upgrade_self()|
|`remove_self`|fn|pub|2034-2037|def remove_self()|
|`cmd_aa`|fn|pub|2042-2049|def cmd_aa(extra)|
|`cmd_ra`|fn|pub|2054-2077|def cmd_ra(extra)|
|`cmd_ar`|fn|pub|2082-2096|def cmd_ar(extra)|
|`cmd_br`|fn|pub|2101-2104|def cmd_br(extra)|
|`cmd_bd`|fn|pub|2109-2112|def cmd_bd(extra)|
|`cmd_ck`|fn|pub|2117-2120|def cmd_ck(extra)|
|`_ensure_commit_ready`|fn|priv|2125-2138|def _ensure_commit_ready(alias)|
|`cmd_cm`|fn|pub|2143-2148|def cmd_cm(extra)|
|`cmd_wip`|fn|pub|2153-2165|def cmd_wip(extra)|
|`cmd_release`|fn|pub|2170-2192|def cmd_release(extra)|
|`cmd_new`|fn|pub|2197-2200|def cmd_new(extra)|
|`cmd_refactor`|fn|pub|2205-2208|def cmd_refactor(extra)|
|`cmd_fix`|fn|pub|2213-2216|def cmd_fix(extra)|
|`cmd_change`|fn|pub|2221-2224|def cmd_change(extra)|
|`cmd_implement`|fn|pub|2229-2232|def cmd_implement(extra)|
|`cmd_docs`|fn|pub|2237-2240|def cmd_docs(extra)|
|`cmd_style`|fn|pub|2245-2248|def cmd_style(extra)|
|`cmd_revert`|fn|pub|2253-2256|def cmd_revert(extra)|
|`cmd_misc`|fn|pub|2261-2264|def cmd_misc(extra)|
|`cmd_cover`|fn|pub|2269-2272|def cmd_cover(extra)|
|`cmd_co`|fn|pub|2277-2280|def cmd_co(extra)|
|`cmd_d`|fn|pub|2285-2292|def cmd_d(extra)|
|`cmd_dcc`|fn|pub|2297-2300|def cmd_dcc(extra)|
|`cmd_dccc`|fn|pub|2305-2308|def cmd_dccc(extra)|
|`cmd_de`|fn|pub|2313-2316|def cmd_de(extra)|
|`cmd_di`|fn|pub|2321-2324|def cmd_di(extra)|
|`cmd_diyou`|fn|pub|2329-2332|def cmd_diyou(extra)|
|`cmd_dime`|fn|pub|2337-2340|def cmd_dime(extra)|
|`cmd_dwc`|fn|pub|2345-2348|def cmd_dwc(extra)|
|`cmd_dwcc`|fn|pub|2353-2356|def cmd_dwcc(extra)|
|`cmd_ed`|fn|pub|2361-2370|def cmd_ed(extra)|
|`cmd_fe`|fn|pub|2375-2378|def cmd_fe(extra)|
|`cmd_feall`|fn|pub|2383-2386|def cmd_feall(extra)|
|`cmd_gp`|fn|pub|2391-2394|def cmd_gp(extra)|
|`cmd_gr`|fn|pub|2399-2402|def cmd_gr(extra)|
|`cmd_str`|fn|pub|2407-2436|def cmd_str(extra)|
|`cmd_lb`|fn|pub|2441-2444|def cmd_lb(extra)|
|`cmd_lg`|fn|pub|2449-2462|def cmd_lg(extra)|
|`cmd_lh`|fn|pub|2467-2470|def cmd_lh(extra)|
|`cmd_ll`|fn|pub|2475-2487|def cmd_ll(extra)|
|`cmd_lm`|fn|pub|2492-2495|def cmd_lm(extra)|
|`cmd_lt`|fn|pub|2501-2520|def cmd_lt(extra)|
|`cmd_me`|fn|pub|2525-2528|def cmd_me(extra)|
|`cmd_pl`|fn|pub|2533-2536|def cmd_pl(extra)|
|`cmd_pt`|fn|pub|2541-2544|def cmd_pt(extra)|
|`cmd_pu`|fn|pub|2549-2552|def cmd_pu(extra)|
|`cmd_rf`|fn|pub|2557-2560|def cmd_rf(extra)|
|`cmd_rmtg`|fn|pub|2565-2575|def cmd_rmtg(extra)|
|`cmd_rmloc`|fn|pub|2580-2583|def cmd_rmloc(extra)|
|`cmd_rmstg`|fn|pub|2588-2591|def cmd_rmstg(extra)|
|`cmd_rmunt`|fn|pub|2596-2599|def cmd_rmunt(extra)|
|`cmd_rs`|fn|pub|2604-2607|def cmd_rs(extra)|
|`cmd_rssft`|fn|pub|2612-2615|def cmd_rssft(extra)|
|`cmd_rsmix`|fn|pub|2620-2623|def cmd_rsmix(extra)|
|`cmd_rshrd`|fn|pub|2628-2631|def cmd_rshrd(extra)|
|`cmd_rsmrg`|fn|pub|2636-2639|def cmd_rsmrg(extra)|
|`cmd_rskep`|fn|pub|2644-2647|def cmd_rskep(extra)|
|`cmd_st`|fn|pub|2652-2655|def cmd_st(extra)|
|`cmd_tg`|fn|pub|2660-2663|def cmd_tg(extra)|
|`cmd_unstg`|fn|pub|2668-2671|def cmd_unstg(extra)|
|`cmd_ver`|fn|pub|2676-2702|def cmd_ver(extra)|
|`cmd_chver`|fn|pub|2707-2779|def cmd_chver(extra)|
|`cmd_major`|fn|pub|2788-2792|def cmd_major(extra)|
|`cmd_minor`|fn|pub|2801-2805|def cmd_minor(extra)|
|`cmd_patch`|fn|pub|2814-2818|def cmd_patch(extra)|
|`cmd_backup`|fn|pub|2826-2836|def cmd_backup(extra)|
|`cmd_changelog`|fn|pub|2846-2878|def cmd_changelog(extra)|
|`COMMANDS`|var|pub|2881||
|`print_command_help`|fn|pub|2954-2960|def print_command_help(name, width=None)|
|`print_all_help`|fn|pub|2964-2998|def print_all_help()|
|`main`|fn|pub|3004-3054|def main(argv=None, *, check_updates: bool = True)|
