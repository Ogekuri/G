...
# core.py | Python | 3971L | 230 symbols | 18 imports | 1011 comments
...
- var `RESET_HELP_COMMANDS = {"rs", "rshrd", "rskep", "rsmix", "rsmrg", "rssft"}` (L696)
- Brief: Constant `RESET_HELP_COMMANDS` used by CLI runtime paths and policies.
### fn `def _to_args(extra)` `priv` (L703-706)
...
### class `class CommandExecutionError(RuntimeError)` : RuntimeError (L708-749)
...
- fn `def __init__(self, exc: subprocess.CalledProcessError)` `priv` (L713-720)
  - Brief: Execute `__init__` runtime logic for Git-Alias CLI.
  - Param: self Input parameter consumed by `__init__`.
  - Param: exc Input parameter consumed by `__init__`.
  - Return: Result emitted by `__init__` according to command contract.
- fn `def _format_message(self) -> str` `priv` (L724-734)
...
- fn `def _decode_stream(data) -> str` `priv` (L739-749)
...
### fn `def _run_checked(*popenargs, **kwargs)` `priv` (L755-762)
...
### class `class VersionDetectionError(RuntimeError)` : RuntimeError (L764-767)
- Brief: Class `VersionDetectionError` models a typed runtime container/error boundary.

### class `class ReleaseError(RuntimeError)` : RuntimeError (L769-772)
- Brief: Class `ReleaseError` models a typed runtime container/error boundary.

### fn `def run_git_cmd(base_args, extra=None, cwd=None, **kwargs)` (L780-784)
...
### fn `def capture_git_output(base_args, cwd=None)` (L790-794)
...
### fn `def run_command(cmd, cwd=None)` (L800-803)
...
### fn `def run_git_text(args, cwd=None, check=True)` (L810-827)
...
### fn `def run_shell(command, cwd=None)` (L833-836)
...
### fn `def _git_status_lines()` `priv` (L840-852)
...
### fn `def has_unstaged_changes(status_lines=None)` (L857-868)
...
### fn `def has_staged_changes(status_lines=None)` (L873-882)
...
- var `WIP_MESSAGE_RE = re.compile(r"^wip: work in progress\.$")` (L888)
- Brief: Constant `WIP_MESSAGE_RE` used by CLI runtime paths and policies.
### fn `def _refresh_remote_refs()` `priv` (L894-905)
...
### fn `def _branch_remote_divergence(branch_key, remote="origin")` `priv` (L911-929)
...
### fn `def has_remote_branch_updates(branch_key, remote="origin")` (L935-939)
...
### fn `def has_remote_develop_updates()` (L943-946)
...
### fn `def has_remote_master_updates()` (L950-953)
...
### fn `def _head_commit_message()` `priv` (L957-963)
...
### fn `def _head_commit_hash()` `priv` (L967-973)
...
### fn `def _commit_exists_in_branch(commit_hash, branch_name)` `priv` (L979-991)
...
### fn `def _should_amend_existing_commit()` `priv` (L995-1010)
...
### fn `def is_inside_git_repo()` (L1014-1021)
...
### class `class TagInfo` (L1026-1034)
...
### fn `def _latest_supported_tag_name(tags: List[TagInfo]) -> Optional[str]` `priv` (L1084-1087)
...
### fn `def _is_minor_release_tag(tag_name: str) -> bool` `priv` (L1095-1102)
...
### fn `def _latest_patch_tag_after(all_tags: List[TagInfo], last_minor: Optional[TagInfo]) -> Optional[TagInfo]` `priv` (L1111-1120)
...
### fn `def list_tags_sorted_by_date(repo_root: Path, merged_ref: Optional[str] = None) -> List[TagInfo]` (L1126-1146)
...
### fn `def git_log_subjects(repo_root: Path, rev_range: str) -> List[str]` (L1153-1164)
...
### fn `def parse_conventional_commit(message: str) -> Optional[Tuple[str, Optional[str], bool, str]]` (L1170-1182)
...
### fn `def _format_changelog_description(desc: str) -> List[str]` `priv` (L1189-1200)
...
### fn `def categorize_commit(subject: str) -> Tuple[Optional[str], str]` (L1208-1237)
...
### fn `def _extract_release_version(subject: str) -> Optional[str]` `priv` (L1242-1252)
...
### fn `def _is_release_marker_commit(subject: str) -> bool` `priv` (L1257-1260)
...
### fn `def generate_section_for_range(repo_root: Path, title: str, date_s: str, rev_range: str, expected_version: Optional[str] = None) -> Optional[str]` (L1269-1307)
...
### fn `def _get_remote_name_for_branch(branch_name: str, repo_root: Path) -> str` `priv` (L1316-1324)
...
### fn `def _extract_owner_repo(remote_url: str) -> Optional[Tuple[str, str]]` `priv` (L1331-1355)
...
### fn `def _canonical_origin_base(repo_root: Path) -> Optional[str]` `priv` (L1365-1378)
...
### fn `def get_origin_compare_url(base_url: Optional[str], prev_tag: Optional[str], tag: str) -> Optional[str]` (L1385-1392)
...
### fn `def get_release_page_url(base_url: Optional[str], tag: str) -> Optional[str]` (L1398-1403)
...
### fn `def build_history_section(` (L1411-1415)
...
### fn `def generate_changelog_document(repo_root: Path, include_patch: bool, disable_history: bool = False) -> str` (L1459-1520)
...
### class `class VersionRuleContext` `@dataclass(frozen=True)` (L1531-1538)

### fn `def _normalize_version_rule_pattern(pattern: str) -> str` `priv` (L1544-1555)
...
### fn `def _build_version_file_inventory(root: Path) -> List[Tuple[Path, str]]` `priv` (L1561-1588)
...
### fn `def _collect_version_files(root, pattern, *, inventory=None)` `priv` (L1596-1613)
...
### fn `def _is_version_path_excluded(relative_path: str) -> bool` `priv` (L1618-1621)
...
### fn `def _iter_versions_in_text(text, compiled_regexes)` `priv` (L1627-1638)
...
### fn `def _read_version_file_text(file_path: Path, text_cache: Optional[Dict[Path, str]] = None) -> Optional[str]` `priv` (L1645-1659)
...
### fn `def _prepare_version_rule_contexts(` `priv` (L1668-1669)
...
### fn `def _determine_canonical_version(` `priv` (L1712-1719)
...
### fn `def _parse_semver_tuple(text: str) -> Optional[Tuple[int, int, int]]` `priv` (L1764-1770)
...
### fn `def _replace_versions_in_text(text, compiled_regex, replacement)` `priv` (L1777-1792)
...
### fn `def _current_branch_name()` `priv` (L1796-1808)
...
### fn `def _ref_exists(ref_name)` `priv` (L1813-1822)
...
### fn `def _local_branch_exists(branch_name)` `priv` (L1827-1830)
...
### fn `def _remote_branch_exists(branch_name)` `priv` (L1835-1838)
...
### fn `def _ensure_release_prerequisites()` `priv` (L1842-1869)
...
### fn `def _bump_semver_version(current_version, level)` `priv` (L1875-1893)
...
### fn `def _run_release_step(level, step_name, action)` `priv` (L1900-1920)
...
### fn `def _create_release_commit_for_flow(target_version)` `priv` (L1925-1930)
...
### fn `def _push_branch_with_tags(branch_name)` `priv` (L1936-1940)
...
### fn `def _execute_release_flow(level, changelog_args=None)` `priv` (L1956-2003)
...
### fn `def _execute_backup_flow()` `priv` (L2011-2026)
...
### fn `def _run_release_command(level, changelog_args=None)` `priv` (L2032-2047)
...
### fn `def _run_backup_command()` `priv` (L2052-2059)
...
### fn `def _run_reset_with_help(base_args, extra)` `priv` (L2065-2072)
...
### fn `def _reject_extra_arguments(extra, alias)` `priv` (L2078-2084)
...
### fn `def _parse_release_flags(extra, alias)` `priv` (L2090-2108)
...
### fn `def _prepare_commit_message(extra, alias)` `priv` (L2114-2124)
...
### fn `def _normalize_conventional_description(description: str) -> str` `priv` (L2131-2141)
...
### fn `def _build_conventional_message(kind: str, extra, alias: str) -> str` `priv` (L2150-2168)
...
### fn `def _run_conventional_commit(kind: str, alias: str, extra)` `priv` (L2175-2180)
...
### fn `def _execute_commit(message, alias, allow_amend=True)` `priv` (L2187-2216)
...
### fn `def upgrade_self()` (L2220-2233)
...
### fn `def cmd_aa(extra)` (L2245-2252)
...
### fn `def cmd_ra(extra)` (L2257-2280)
...
### fn `def cmd_ar(extra)` (L2285-2299)
...
### fn `def cmd_br(extra)` (L2304-2307)
...
### fn `def cmd_bd(extra)` (L2312-2315)
...
### fn `def cmd_ck(extra)` (L2320-2323)
...
### fn `def _ensure_commit_ready(alias)` `priv` (L2328-2341)
...
### fn `def cmd_cm(extra)` (L2346-2351)
...
### fn `def cmd_wip(extra)` (L2356-2368)
...
### fn `def cmd_release(extra)` (L2373-2395)
...
### fn `def cmd_new(extra)` (L2400-2403)
...
### fn `def cmd_refactor(extra)` (L2408-2411)
...
### fn `def cmd_fix(extra)` (L2416-2419)
...
### fn `def cmd_change(extra)` (L2424-2427)
...
### fn `def cmd_implement(extra)` (L2432-2435)
...
### fn `def cmd_docs(extra)` (L2440-2443)
...
### fn `def cmd_style(extra)` (L2448-2451)
...
### fn `def cmd_revert(extra)` (L2456-2459)
...
### fn `def cmd_misc(extra)` (L2464-2467)
...
### fn `def cmd_cover(extra)` (L2472-2475)
...
### fn `def cmd_co(extra)` (L2480-2483)
...
### fn `def cmd_dwd(extra)` (L2577-2582)
...
### fn `def cmd_ed(extra)` (L2587-2596)
...
### fn `def cmd_fe(extra)` (L2601-2604)
...
### fn `def cmd_feall(extra)` (L2609-2612)
...
### fn `def cmd_gp(extra)` (L2617-2620)
...
### fn `def cmd_gr(extra)` (L2625-2628)
...
### fn `def _overview_work_prefix_color(worktree_state: str) -> str` `priv` (L2674-2681)
...
### fn `def _overview_logical_branch_name(` `priv` (L2689-2693)
...
### fn `def _overview_current_branch_display(` `priv` (L2712-2717)
...
### fn `def _overview_ref_is_available(ref_name: str) -> bool` `priv` (L2739-2748)
...
### fn `def _overview_ref_latest_subject(ref_name: str) -> str` `priv` (L2754-2764)
...
### fn `def _overview_discovered_branch_refs() -> List[str]` `priv` (L2769-2792)
...
### fn `def _overview_branch_summary_lines(` `priv` (L2811-2822)
...
### fn `def _overview_relation_state(ahead: int, behind: int) -> str` `priv` (L2855-2864)
...
### fn `def _overview_worktree_state(status_lines=None) -> str` `priv` (L2869-2881)
...
### fn `def _overview_distance_text(is_ahead: bool, count: int) -> str` `priv` (L2887-2895)
...
### fn `def _overview_compare_refs(base_ref: str, target_ref: str, label: str) -> str` `priv` (L2902-2942)
...
### fn `def _overview_ascii_topology_lines(` `priv` (L2963-2974)
...
### fn `def _overview_current_branch_state_lines(current_branch_display: str) -> List[str]` `priv` (L3064-3083)
...
### fn `def cmd_str(extra)` (L3237-3266)
...
### fn `def cmd_lb(extra)` (L3285-3288)
...
### fn `def cmd_lg(extra)` (L3293-3306)
...
### fn `def cmd_lh(extra)` (L3311-3314)
...
### fn `def cmd_ll(extra)` (L3319-3331)
...
### fn `def cmd_lm(extra)` (L3336-3339)
...
### fn `def cmd_ls(extra)` (L3345-3348)
...
### fn `def cmd_lsi(extra)` (L3354-3360)
...
### fn `def cmd_lsa(extra)` (L3366-3369)
...
### fn `def cmd_lt(extra)` (L3375-3394)
...
### fn `def cmd_me(extra)` (L3399-3402)
...
### fn `def cmd_pl(extra)` (L3407-3410)
...
### fn `def cmd_pt(extra)` (L3415-3418)
...
### fn `def cmd_pu(extra)` (L3423-3426)
...
### fn `def cmd_rf(extra)` (L3431-3434)
...
### fn `def cmd_rmtg(extra)` (L3439-3449)
...
### fn `def cmd_rmloc(extra)` (L3454-3457)
...
### fn `def cmd_rmstg(extra)` (L3462-3465)
...
### fn `def cmd_rmunt(extra)` (L3470-3473)
...
### fn `def cmd_rs(extra)` (L3478-3481)
...
### fn `def cmd_rssft(extra)` (L3486-3489)
...
### fn `def cmd_rsmix(extra)` (L3494-3497)
...
### fn `def cmd_rshrd(extra)` (L3502-3505)
...
### fn `def cmd_rsmrg(extra)` (L3510-3513)
...
### fn `def cmd_rskep(extra)` (L3518-3521)
...
### fn `def cmd_st(extra)` (L3526-3529)
...
### fn `def cmd_tg(extra)` (L3534-3537)
...
### fn `def cmd_unstg(extra)` (L3542-3545)
...
### fn `def cmd_wt(extra)` (L3550-3553)
...
### fn `def cmd_wtl(extra)` (L3558-3561)
...
### fn `def cmd_wtp(extra)` (L3566-3569)
...
### fn `def cmd_wtr(extra)` (L3574-3577)
...
### fn `def cmd_ver(extra)` (L3582-3608)
...
### fn `def cmd_chver(extra)` (L3613-3685)
...
### fn `def cmd_major(extra)` (L3694-3698)
...
### fn `def cmd_minor(extra)` (L3707-3711)
...
### fn `def cmd_patch(extra)` (L3720-3724)
...
### fn `def cmd_backup(extra)` (L3732-3742)
...
### fn `def cmd_changelog(extra)` (L3752-3784)
...
### fn `def print_command_help(name, width=None)` (L3871-3877)
...
### fn `def print_all_help()` (L3881-3915)
...
### fn `def main(argv=None, *, check_updates: bool = True)` (L3921-3971)
...
|`RESET_HELP_COMMANDS`|var|pub|696||
|`_to_args`|fn|priv|703-706|def _to_args(extra)|
|`CommandExecutionError`|class|pub|708-749|class CommandExecutionError(RuntimeError)|
|`CommandExecutionError.__init__`|fn|priv|713-720|def __init__(self, exc: subprocess.CalledProcessError)|
|`CommandExecutionError._format_message`|fn|priv|724-734|def _format_message(self) -> str|
|`CommandExecutionError._decode_stream`|fn|priv|739-749|def _decode_stream(data) -> str|
|`_run_checked`|fn|priv|755-762|def _run_checked(*popenargs, **kwargs)|
|`VersionDetectionError`|class|pub|764-767|class VersionDetectionError(RuntimeError)|
|`ReleaseError`|class|pub|769-772|class ReleaseError(RuntimeError)|
|`run_git_cmd`|fn|pub|780-784|def run_git_cmd(base_args, extra=None, cwd=None, **kwargs)|
|`capture_git_output`|fn|pub|790-794|def capture_git_output(base_args, cwd=None)|
|`run_command`|fn|pub|800-803|def run_command(cmd, cwd=None)|
|`run_git_text`|fn|pub|810-827|def run_git_text(args, cwd=None, check=True)|
|`run_shell`|fn|pub|833-836|def run_shell(command, cwd=None)|
|`_git_status_lines`|fn|priv|840-852|def _git_status_lines()|
|`has_unstaged_changes`|fn|pub|857-868|def has_unstaged_changes(status_lines=None)|
|`has_staged_changes`|fn|pub|873-882|def has_staged_changes(status_lines=None)|
|`WIP_MESSAGE_RE`|var|pub|888||
|`_refresh_remote_refs`|fn|priv|894-905|def _refresh_remote_refs()|
|`_branch_remote_divergence`|fn|priv|911-929|def _branch_remote_divergence(branch_key, remote="origin")|
|`has_remote_branch_updates`|fn|pub|935-939|def has_remote_branch_updates(branch_key, remote="origin")|
|`has_remote_develop_updates`|fn|pub|943-946|def has_remote_develop_updates()|
|`has_remote_master_updates`|fn|pub|950-953|def has_remote_master_updates()|
|`_head_commit_message`|fn|priv|957-963|def _head_commit_message()|
|`_head_commit_hash`|fn|priv|967-973|def _head_commit_hash()|
|`_commit_exists_in_branch`|fn|priv|979-991|def _commit_exists_in_branch(commit_hash, branch_name)|
|`_should_amend_existing_commit`|fn|priv|995-1010|def _should_amend_existing_commit()|
|`is_inside_git_repo`|fn|pub|1014-1021|def is_inside_git_repo()|
|`TagInfo`|class|pub|1026-1034|class TagInfo|
|`DELIM`|var|pub|1037||
|`RECORD`|var|pub|1040||
|`SEMVER_RE`|var|pub|1056||
|`SECTION_EMOJI`|var|pub|1059||
|`_tag_semver_tuple`|fn|priv|1076-1079|def _tag_semver_tuple(tag_name: str) -> Optional[Tuple[in...|
|`_latest_supported_tag_name`|fn|priv|1084-1087|def _latest_supported_tag_name(tags: List[TagInfo]) -> Op...|
|`_is_minor_release_tag`|fn|priv|1095-1102|def _is_minor_release_tag(tag_name: str) -> bool|
|`_latest_patch_tag_after`|fn|priv|1111-1120|def _latest_patch_tag_after(all_tags: List[TagInfo], last...|
|`list_tags_sorted_by_date`|fn|pub|1126-1146|def list_tags_sorted_by_date(repo_root: Path, merged_ref:...|
|`git_log_subjects`|fn|pub|1153-1164|def git_log_subjects(repo_root: Path, rev_range: str) -> ...|
|`parse_conventional_commit`|fn|pub|1170-1182|def parse_conventional_commit(message: str) -> Optional[T...|
|`_format_changelog_description`|fn|priv|1189-1200|def _format_changelog_description(desc: str) -> List[str]|
|`categorize_commit`|fn|pub|1208-1237|def categorize_commit(subject: str) -> Tuple[Optional[str...|
|`_extract_release_version`|fn|priv|1242-1252|def _extract_release_version(subject: str) -> Optional[str]|
|`_is_release_marker_commit`|fn|priv|1257-1260|def _is_release_marker_commit(subject: str) -> bool|
|`generate_section_for_range`|fn|pub|1269-1307|def generate_section_for_range(repo_root: Path, title: st...|
|`_get_remote_name_for_branch`|fn|priv|1316-1324|def _get_remote_name_for_branch(branch_name: str, repo_ro...|
|`_extract_owner_repo`|fn|priv|1331-1355|def _extract_owner_repo(remote_url: str) -> Optional[Tupl...|
|`_canonical_origin_base`|fn|priv|1365-1378|def _canonical_origin_base(repo_root: Path) -> Optional[str]|
|`get_origin_compare_url`|fn|pub|1385-1392|def get_origin_compare_url(base_url: Optional[str], prev_...|
|`get_release_page_url`|fn|pub|1398-1403|def get_release_page_url(base_url: Optional[str], tag: st...|
|`build_history_section`|fn|pub|1411-1415|def build_history_section(|
|`generate_changelog_document`|fn|pub|1459-1520|def generate_changelog_document(repo_root: Path, include_...|
|`VersionRuleContext`|class|pub|1531-1538|class VersionRuleContext|
|`_normalize_version_rule_pattern`|fn|priv|1544-1555|def _normalize_version_rule_pattern(pattern: str) -> str|
|`_build_version_file_inventory`|fn|priv|1561-1588|def _build_version_file_inventory(root: Path) -> List[Tup...|
|`_collect_version_files`|fn|priv|1596-1613|def _collect_version_files(root, pattern, *, inventory=None)|
|`_is_version_path_excluded`|fn|priv|1618-1621|def _is_version_path_excluded(relative_path: str) -> bool|
|`_iter_versions_in_text`|fn|priv|1627-1638|def _iter_versions_in_text(text, compiled_regexes)|
|`_read_version_file_text`|fn|priv|1645-1659|def _read_version_file_text(file_path: Path, text_cache: ...|
|`_prepare_version_rule_contexts`|fn|priv|1668-1669|def _prepare_version_rule_contexts(|
|`_determine_canonical_version`|fn|priv|1712-1719|def _determine_canonical_version(|
|`_parse_semver_tuple`|fn|priv|1764-1770|def _parse_semver_tuple(text: str) -> Optional[Tuple[int,...|
|`_replace_versions_in_text`|fn|priv|1777-1792|def _replace_versions_in_text(text, compiled_regex, repla...|
|`_current_branch_name`|fn|priv|1796-1808|def _current_branch_name()|
|`_ref_exists`|fn|priv|1813-1822|def _ref_exists(ref_name)|
|`_local_branch_exists`|fn|priv|1827-1830|def _local_branch_exists(branch_name)|
|`_remote_branch_exists`|fn|priv|1835-1838|def _remote_branch_exists(branch_name)|
|`_ensure_release_prerequisites`|fn|priv|1842-1869|def _ensure_release_prerequisites()|
|`_bump_semver_version`|fn|priv|1875-1893|def _bump_semver_version(current_version, level)|
|`_run_release_step`|fn|priv|1900-1920|def _run_release_step(level, step_name, action)|
|`_create_release_commit_for_flow`|fn|priv|1925-1930|def _create_release_commit_for_flow(target_version)|
|`_push_branch_with_tags`|fn|priv|1936-1940|def _push_branch_with_tags(branch_name)|
|`_execute_release_flow`|fn|priv|1956-2003|def _execute_release_flow(level, changelog_args=None)|
|`_execute_backup_flow`|fn|priv|2011-2026|def _execute_backup_flow()|
|`_run_release_command`|fn|priv|2032-2047|def _run_release_command(level, changelog_args=None)|
|`_run_backup_command`|fn|priv|2052-2059|def _run_backup_command()|
|`_run_reset_with_help`|fn|priv|2065-2072|def _run_reset_with_help(base_args, extra)|
|`_reject_extra_arguments`|fn|priv|2078-2084|def _reject_extra_arguments(extra, alias)|
|`_parse_release_flags`|fn|priv|2090-2108|def _parse_release_flags(extra, alias)|
|`_prepare_commit_message`|fn|priv|2114-2124|def _prepare_commit_message(extra, alias)|
|`_normalize_conventional_description`|fn|priv|2131-2141|def _normalize_conventional_description(description: str)...|
|`_build_conventional_message`|fn|priv|2150-2168|def _build_conventional_message(kind: str, extra, alias: ...|
|`_run_conventional_commit`|fn|priv|2175-2180|def _run_conventional_commit(kind: str, alias: str, extra)|
|`_execute_commit`|fn|priv|2187-2216|def _execute_commit(message, alias, allow_amend=True)|
|`upgrade_self`|fn|pub|2220-2233|def upgrade_self()|
|`remove_self`|fn|pub|2237-2240|def remove_self()|
|`cmd_aa`|fn|pub|2245-2252|def cmd_aa(extra)|
|`cmd_ra`|fn|pub|2257-2280|def cmd_ra(extra)|
|`cmd_ar`|fn|pub|2285-2299|def cmd_ar(extra)|
|`cmd_br`|fn|pub|2304-2307|def cmd_br(extra)|
|`cmd_bd`|fn|pub|2312-2315|def cmd_bd(extra)|
|`cmd_ck`|fn|pub|2320-2323|def cmd_ck(extra)|
|`_ensure_commit_ready`|fn|priv|2328-2341|def _ensure_commit_ready(alias)|
|`cmd_cm`|fn|pub|2346-2351|def cmd_cm(extra)|
|`cmd_wip`|fn|pub|2356-2368|def cmd_wip(extra)|
|`cmd_release`|fn|pub|2373-2395|def cmd_release(extra)|
|`cmd_new`|fn|pub|2400-2403|def cmd_new(extra)|
|`cmd_refactor`|fn|pub|2408-2411|def cmd_refactor(extra)|
|`cmd_fix`|fn|pub|2416-2419|def cmd_fix(extra)|
|`cmd_change`|fn|pub|2424-2427|def cmd_change(extra)|
|`cmd_implement`|fn|pub|2432-2435|def cmd_implement(extra)|
|`cmd_docs`|fn|pub|2440-2443|def cmd_docs(extra)|
|`cmd_style`|fn|pub|2448-2451|def cmd_style(extra)|
|`cmd_revert`|fn|pub|2456-2459|def cmd_revert(extra)|
|`cmd_misc`|fn|pub|2464-2467|def cmd_misc(extra)|
|`cmd_cover`|fn|pub|2472-2475|def cmd_cover(extra)|
...
|`cmd_dwd`|fn|pub|2577-2582|def cmd_dwd(extra)|
|`cmd_ed`|fn|pub|2587-2596|def cmd_ed(extra)|
|`cmd_fe`|fn|pub|2601-2604|def cmd_fe(extra)|
|`cmd_feall`|fn|pub|2609-2612|def cmd_feall(extra)|
|`cmd_gp`|fn|pub|2617-2620|def cmd_gp(extra)|
|`cmd_gr`|fn|pub|2625-2628|def cmd_gr(extra)|
|`OVERVIEW_COLOR_RESET`|var|pub|2630||
|`OVERVIEW_COLOR_SECTION_PURPLE`|var|pub|2632||
|`OVERVIEW_COLOR_AHEAD`|var|pub|2634||
|`OVERVIEW_COLOR_BEHIND`|var|pub|2636||
|`OVERVIEW_COLOR_LABEL`|var|pub|2638||
|`OVERVIEW_COLOR_WHITE`|var|pub|2640||
|`OVERVIEW_COLOR_WHITE_BOLD`|var|pub|2642||
|`OVERVIEW_SECTION_TEMPLATE`|var|pub|2644||
|`OVERVIEW_SUBSECTION_TEMPLATE`|var|pub|2646||
|`OVERVIEW_DISTANCE_TEMPLATE`|var|pub|2648||
|`_overview_branch_identifier`|fn|priv|2657-2660|def _overview_branch_identifier(|
|`_overview_work_prefix_color`|fn|priv|2674-2681|def _overview_work_prefix_color(worktree_state: str) -> str|
|`_overview_logical_branch_name`|fn|priv|2689-2693|def _overview_logical_branch_name(|
|`_overview_current_branch_display`|fn|priv|2712-2717|def _overview_current_branch_display(|
|`_overview_ref_is_available`|fn|priv|2739-2748|def _overview_ref_is_available(ref_name: str) -> bool|
|`_overview_ref_latest_subject`|fn|priv|2754-2764|def _overview_ref_latest_subject(ref_name: str) -> str|
|`_overview_discovered_branch_refs`|fn|priv|2769-2792|def _overview_discovered_branch_refs() -> List[str]|
|`_overview_branch_summary_lines`|fn|priv|2811-2822|def _overview_branch_summary_lines(|
|`_overview_relation_state`|fn|priv|2855-2864|def _overview_relation_state(ahead: int, behind: int) -> str|
|`_overview_worktree_state`|fn|priv|2869-2881|def _overview_worktree_state(status_lines=None) -> str|
|`_overview_distance_text`|fn|priv|2887-2895|def _overview_distance_text(is_ahead: bool, count: int) -...|
|`_overview_compare_refs`|fn|priv|2902-2942|def _overview_compare_refs(base_ref: str, target_ref: str...|
|`_overview_ascii_topology_lines`|fn|priv|2963-2974|def _overview_ascii_topology_lines(|
|`_overview_current_branch_state_lines`|fn|priv|3064-3083|def _overview_current_branch_state_lines(current_branch_d...|
|`cmd_o`|fn|pub|3089-3232|def cmd_o(extra)|
|`cmd_str`|fn|pub|3237-3266|def cmd_str(extra)|
|`cmd_l`|fn|pub|3275-3280|def cmd_l(extra)|
|`cmd_lb`|fn|pub|3285-3288|def cmd_lb(extra)|
|`cmd_lg`|fn|pub|3293-3306|def cmd_lg(extra)|
|`cmd_lh`|fn|pub|3311-3314|def cmd_lh(extra)|
|`cmd_ll`|fn|pub|3319-3331|def cmd_ll(extra)|
|`cmd_lm`|fn|pub|3336-3339|def cmd_lm(extra)|
|`cmd_ls`|fn|pub|3345-3348|def cmd_ls(extra)|
|`cmd_lsi`|fn|pub|3354-3360|def cmd_lsi(extra)|
|`cmd_lsa`|fn|pub|3366-3369|def cmd_lsa(extra)|
|`cmd_lt`|fn|pub|3375-3394|def cmd_lt(extra)|
|`cmd_me`|fn|pub|3399-3402|def cmd_me(extra)|
|`cmd_pl`|fn|pub|3407-3410|def cmd_pl(extra)|
|`cmd_pt`|fn|pub|3415-3418|def cmd_pt(extra)|
|`cmd_pu`|fn|pub|3423-3426|def cmd_pu(extra)|
|`cmd_rf`|fn|pub|3431-3434|def cmd_rf(extra)|
|`cmd_rmtg`|fn|pub|3439-3449|def cmd_rmtg(extra)|
|`cmd_rmloc`|fn|pub|3454-3457|def cmd_rmloc(extra)|
|`cmd_rmstg`|fn|pub|3462-3465|def cmd_rmstg(extra)|
|`cmd_rmunt`|fn|pub|3470-3473|def cmd_rmunt(extra)|
|`cmd_rs`|fn|pub|3478-3481|def cmd_rs(extra)|
|`cmd_rssft`|fn|pub|3486-3489|def cmd_rssft(extra)|
|`cmd_rsmix`|fn|pub|3494-3497|def cmd_rsmix(extra)|
|`cmd_rshrd`|fn|pub|3502-3505|def cmd_rshrd(extra)|
|`cmd_rsmrg`|fn|pub|3510-3513|def cmd_rsmrg(extra)|
|`cmd_rskep`|fn|pub|3518-3521|def cmd_rskep(extra)|
|`cmd_st`|fn|pub|3526-3529|def cmd_st(extra)|
|`cmd_tg`|fn|pub|3534-3537|def cmd_tg(extra)|
|`cmd_unstg`|fn|pub|3542-3545|def cmd_unstg(extra)|
|`cmd_wt`|fn|pub|3550-3553|def cmd_wt(extra)|
|`cmd_wtl`|fn|pub|3558-3561|def cmd_wtl(extra)|
|`cmd_wtp`|fn|pub|3566-3569|def cmd_wtp(extra)|
|`cmd_wtr`|fn|pub|3574-3577|def cmd_wtr(extra)|
|`cmd_ver`|fn|pub|3582-3608|def cmd_ver(extra)|
|`cmd_chver`|fn|pub|3613-3685|def cmd_chver(extra)|
|`cmd_major`|fn|pub|3694-3698|def cmd_major(extra)|
|`cmd_minor`|fn|pub|3707-3711|def cmd_minor(extra)|
|`cmd_patch`|fn|pub|3720-3724|def cmd_patch(extra)|
|`cmd_backup`|fn|pub|3732-3742|def cmd_backup(extra)|
|`cmd_changelog`|fn|pub|3752-3784|def cmd_changelog(extra)|
|`COMMANDS`|var|pub|3787||
|`print_command_help`|fn|pub|3871-3877|def print_command_help(name, width=None)|
|`print_all_help`|fn|pub|3881-3915|def print_all_help()|
|`main`|fn|pub|3921-3971|def main(argv=None, *, check_updates: bool = True)|
