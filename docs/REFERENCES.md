...
# core.py | Python | 4719L | 260 symbols | 18 imports | 1208 comments
...
- var `VERSION_CHECK_RATE_LIMIT_IDLE_DELAY_SECONDS = 3600` (L71)
...
### fn `def check_for_newer_version(` (L345-349)
...
### fn `def get_config_path(root=None)` (L489-493)
...
### fn `def get_global_config_path(home=None)` (L498-502)
...
### fn `def _read_config_object(config_path)` `priv` (L507-525)
...
### fn `def _apply_config_values(data, keys)` `priv` (L531-553)
...
### fn `def load_cli_config(root=None, home=None)` (L559-571)
...
### fn `def _write_missing_config_values(config_path, keys, create_parent=False)` `priv` (L578-635)
...
### fn `def write_default_config(root=None, home=None)` (L641-654)
...
### fn `def _editor_base_command()` `priv` (L658-672)
...
### fn `def run_editor_command(args)` (L677-680)
...
### fn `def _config_command_parts(key: str, default_command: str) -> List[str]` `priv` (L688-712)
...
- var `RESET_HELP_COMMANDS = {"rs", "rshrd", "rskep", "rsmix", "rsmrg", "rssft"}` (L881)
- @brief Constant `RESET_HELP_COMMANDS` used by CLI runtime paths and policies.
- var `LSI_DEFAULT_EXCLUDED_DIRS = frozenset(` (L887)
...
- @satisfies REQ-120
- var `LSI_DEFAULT_EXCLUDED_DIR_SUFFIXES = (".egg-info",)` (L924)
...
- @satisfies REQ-122
### fn `def _to_args(extra)` `priv` (L931-934)
...
### class `class CommandExecutionError(RuntimeError)` : RuntimeError (L937-978)
...
- fn `def __init__(self, exc: subprocess.CalledProcessError)` `priv` (L942-949)
  - @brief Execute `__init__` runtime logic for Git-Alias CLI.
  - @param self Input parameter consumed by `__init__`.
  - @param exc Input parameter consumed by `__init__`.
  - @return Result emitted by `__init__` according to command contract.
- fn `def _format_message(self) -> str` `priv` (L953-963)
...
- fn `def _decode_stream(data) -> str` `priv` (L968-978)
...
### fn `def _run_checked(*popenargs, **kwargs)` `priv` (L984-991)
...
### class `class VersionDetectionError(RuntimeError)` : RuntimeError (L994-997)
...
### class `class ReleaseError(RuntimeError)` : RuntimeError (L1000-1003)
...
### fn `def run_git_cmd(base_args, extra=None, cwd=None, **kwargs)` (L1011-1015)
...
### fn `def capture_git_output(base_args, cwd=None)` (L1021-1027)
...
### fn `def run_command(cmd, cwd=None)` (L1033-1036)
...
### fn `def run_git_text(args, cwd=None, check=True)` (L1043-1060)
...
### fn `def run_shell(command, cwd=None)` (L1066-1069)
...
### fn `def _git_status_lines()` `priv` (L1073-1085)
...
### fn `def has_unstaged_changes(status_lines=None)` (L1090-1101)
...
### fn `def has_staged_changes(status_lines=None)` (L1106-1115)
...
- var `WIP_MESSAGE_RE = re.compile(r"^wip: work in progress\.$")` (L1121)
- @brief Constant `WIP_MESSAGE_RE` used by CLI runtime paths and policies.
### fn `def _refresh_remote_refs()` `priv` (L1127-1138)
...
### fn `def _branch_remote_divergence(branch_key, remote="origin")` `priv` (L1144-1164)
...
### fn `def has_remote_branch_updates(branch_key, remote="origin")` (L1170-1174)
...
### fn `def has_remote_develop_updates()` (L1178-1181)
...
### fn `def has_remote_master_updates()` (L1185-1188)
...
### fn `def _head_commit_message()` `priv` (L1192-1198)
...
### fn `def _head_commit_hash()` `priv` (L1202-1208)
...
### fn `def _commit_exists_in_branch(commit_hash, branch_name)` `priv` (L1214-1226)
...
### fn `def _should_amend_existing_commit()` `priv` (L1230-1245)
...
### fn `def is_inside_git_repo()` (L1249-1256)
...
### class `class TagInfo` (L1261-1269)
...
### fn `def _latest_supported_tag_name(tags: List[TagInfo]) -> Optional[str]` `priv` (L1320-1323)
...
### fn `def _is_minor_release_tag(tag_name: str) -> bool` `priv` (L1331-1338)
...
### fn `def _latest_patch_tag_after(` `priv` (L1347-1348)
...
### fn `def list_tags_sorted_by_date(` (L1366-1367)
...
### fn `def git_log_subjects(repo_root: Path, rev_range: str) -> List[str]` (L1397-1408)
...
### fn `def parse_conventional_commit(` (L1414-1415)
...
### fn `def _format_changelog_description(desc: str) -> List[str]` `priv` (L1435-1448)
...
### fn `def categorize_commit(subject: str) -> Tuple[Optional[str], str]` (L1456-1485)
...
### fn `def _extract_release_version(subject: str) -> Optional[str]` `priv` (L1490-1500)
...
### fn `def _is_release_marker_commit(subject: str) -> bool` `priv` (L1505-1508)
...
### fn `def generate_section_for_range(` (L1517-1522)
...
### fn `def _get_remote_name_for_branch(branch_name: str, repo_root: Path) -> str` `priv` (L1570-1578)
...
### fn `def _extract_owner_repo(remote_url: str) -> Optional[Tuple[str, str]]` `priv` (L1586-1610)
...
### fn `def _canonical_origin_base(repo_root: Path) -> Optional[str]` `priv` (L1620-1635)
...
### fn `def get_origin_compare_url(` (L1642-1643)
...
### fn `def get_release_page_url(base_url: Optional[str], tag: str) -> Optional[str]` (L1657-1662)
...
### fn `def build_history_section(` (L1670-1674)
...
### fn `def generate_changelog_document(` (L1724-1725)
...
### class `class VersionRuleContext` `@dataclass(frozen=True)` (L1797-1804)

### fn `def _normalize_version_rule_pattern(pattern: str) -> str` `priv` (L1810-1821)
...
### fn `def _build_version_file_inventory(root: Path) -> List[Tuple[Path, str]]` `priv` (L1827-1854)
...
### fn `def _collect_version_files(root, pattern, *, inventory=None)` `priv` (L1862-1885)
...
### fn `def _is_version_path_excluded(relative_path: str) -> bool` `priv` (L1890-1893)
...
### fn `def _iter_versions_in_text(text, compiled_regexes)` `priv` (L1899-1910)
...
### fn `def _read_version_file_text(` `priv` (L1917-1918)
...
### fn `def _prepare_version_rule_contexts(` `priv` (L1942-1943)
...
### fn `def _determine_canonical_version(` `priv` (L1986-1993)
...
### fn `def _parse_semver_tuple(text: str) -> Optional[Tuple[int, int, int]]` `priv` (L2046-2052)
...
### fn `def _replace_versions_in_text(text, compiled_regex, replacement)` `priv` (L2059-2074)
...
### fn `def _current_branch_name()` `priv` (L2078-2090)
...
### fn `def _ref_exists(ref_name)` `priv` (L2095-2104)
...
### fn `def _local_branch_exists(branch_name)` `priv` (L2109-2112)
...
### fn `def _remote_branch_exists(branch_name)` `priv` (L2117-2120)
...
### fn `def _ensure_release_prerequisites()` `priv` (L2124-2173)
...
### fn `def _bump_semver_version(current_version, level)` `priv` (L2179-2199)
...
### fn `def _run_release_step(level, step_name, action)` `priv` (L2206-2232)
...
### fn `def _create_release_commit_for_flow(target_version)` `priv` (L2237-2242)
...
### fn `def _push_branch_with_tags(branch_name)` `priv` (L2248-2252)
...
### fn `def _execute_release_flow(level, changelog_args=None)` `priv` (L2273-2364)
...
### fn `def _execute_backup_flow()` `priv` (L2372-2389)
...
### fn `def _run_release_command(level, changelog_args=None)` `priv` (L2395-2410)
...
### fn `def _run_backup_command()` `priv` (L2415-2422)
...
### fn `def _run_reset_with_help(base_args, extra)` `priv` (L2428-2435)
...
### fn `def _reject_extra_arguments(extra, alias)` `priv` (L2441-2447)
...
### fn `def _parse_release_flags(extra, alias)` `priv` (L2453-2473)
...
### fn `def _prepare_commit_message(extra, alias)` `priv` (L2479-2489)
...
### fn `def _normalize_conventional_description(description: str) -> str` `priv` (L2496-2506)
...
### fn `def _build_conventional_message(kind: str, extra, alias: str) -> str` `priv` (L2515-2536)
...
### fn `def _run_conventional_commit(kind: str, alias: str, extra)` `priv` (L2554-2559)
...
### fn `def _execute_commit(message, alias, allow_amend=True)` `priv` (L2566-2598)
...
### fn `def _ensure_clean_worktree_and_index(alias: str) -> bool` `priv` (L2606-2622)
...
### fn `def _resolve_rollback_target_commit(target: str) -> str` `priv` (L2630-2640)
...
### fn `def _is_commit_ancestor(ancestor_commit: str, descendant_ref: str) -> bool` `priv` (L2647-2656)
...
### fn `def _is_commit_hash_token(token: str) -> bool` `priv` (L2661-2664)
...
### fn `def _tag_exists(tag_name: str) -> bool` `priv` (L2669-2672)
...
### fn `def _is_linux_platform() -> bool` `priv` (L2677-2680)
...
### fn `def _print_non_linux_management_command(` `priv` (L2687-2689)
...
### fn `def upgrade_self(repo_root: Optional[Path] = None)` (L2703-2710)
...
### fn `def _remove_version_check_cache_artifacts() -> None` `priv` (L2717-2738)
...
### fn `def uninstall_self()` (L2743-2750)
...
### fn `def cmd_aa(extra)` (L2755-2762)
...
### fn `def cmd_ra(extra)` (L2767-2790)
...
### fn `def cmd_ar(extra)` (L2795-2811)
...
### fn `def cmd_br(extra)` (L2816-2819)
...
### fn `def cmd_bd(extra)` (L2824-2827)
...
### fn `def cmd_ck(extra)` (L2832-2835)
...
### fn `def _ensure_commit_ready(alias)` `priv` (L2840-2853)
...
### fn `def _ensure_commit_ready_with_stage(alias)` `priv` (L2871-2885)
...
### fn `def cmd_cm(extra)` (L2890-2895)
...
### fn `def cmd_wip(extra)` (L2905-2917)
...
### fn `def cmd_release(extra)` (L2922-2944)
...
### fn `def cmd_new(extra)` (L2949-2952)
...
### fn `def cmd_refactor(extra)` (L2957-2960)
...
### fn `def cmd_fix(extra)` (L2965-2968)
...
### fn `def cmd_change(extra)` (L2973-2976)
...
### fn `def cmd_implement(extra)` (L2981-2984)
...
### fn `def cmd_docs(extra)` (L2989-2992)
...
### fn `def cmd_style(extra)` (L2997-3000)
...
### fn `def cmd_revert(extra)` (L3005-3008)
...
### fn `def cmd_rollback(extra)` (L3017-3070)
...
### fn `def cmd_misc(extra)` (L3075-3078)
...
### fn `def cmd_cover(extra)` (L3083-3086)
...
### fn `def cmd_co(extra)` (L3091-3094)
...
### fn `def cmd_dc(extra)` (L3099-3108)
...
### fn `def cmd_dcc(extra)` (L3113-3116)
...
### fn `def cmd_dccc(extra)` (L3121-3124)
...
### fn `def cmd_de(extra)` (L3129-3132)
...
### fn `def cmd_di(extra)` (L3137-3140)
...
### fn `def cmd_diyou(extra)` (L3145-3148)
...
### fn `def cmd_dime(extra)` (L3153-3156)
...
### fn `def cmd_dwc(extra)` (L3161-3164)
...
### fn `def cmd_dw(extra)` (L3169-3176)
...
### fn `def cmd_dwcc(extra)` (L3181-3184)
...
### fn `def cmd_dcd(extra)` (L3190-3195)
...
### fn `def cmd_dcm(extra)` (L3201-3206)
...
### fn `def cmd_ddm(extra)` (L3212-3217)
...
### fn `def cmd_ed(extra)` (L3222-3231)
...
### fn `def cmd_fe(extra)` (L3236-3239)
...
### fn `def cmd_feall(extra)` (L3244-3247)
...
### fn `def cmd_gp(extra)` (L3252-3257)
...
### fn `def cmd_gr(extra)` (L3262-3267)
...
### fn `def _overview_work_prefix_color(worktree_state: str) -> str` `priv` (L3313-3320)
...
### fn `def _overview_logical_branch_name(` `priv` (L3328-3332)
...
### fn `def _overview_current_branch_display(` `priv` (L3351-3356)
...
### fn `def _overview_ref_is_available(ref_name: str) -> bool` `priv` (L3378-3387)
...
### fn `def _overview_ref_latest_subject(ref_name: str) -> str` `priv` (L3393-3403)
...
### fn `def _overview_discovered_branch_refs() -> List[str]` `priv` (L3408-3431)
...
### fn `def _overview_branch_summary_lines(` `priv` (L3464-3475)
...
### fn `def _overview_relation_state(ahead: int, behind: int) -> str` `priv` (L3518-3527)
...
### fn `def _overview_worktree_state(status_lines=None) -> str` `priv` (L3532-3544)
...
### fn `def _overview_distance_text(is_ahead: bool, count: int) -> str` `priv` (L3550-3558)
...
### fn `def _overview_compare_refs(base_ref: str, target_ref: str, label: str) -> str` `priv` (L3565-3607)
...
### fn `def _overview_ascii_topology_lines(` `priv` (L3642-3653)
...
### fn `def _overview_current_branch_state_lines(current_branch_display: str) -> List[str]` `priv` (L3745-3764)
...
### fn `def cmd_str(extra)` (L3929-3958)
...
### fn `def cmd_lb(extra)` (L3978-3981)
...
### fn `def cmd_lg(extra)` (L3986-3999)
...
### fn `def cmd_lh(extra)` (L4004-4007)
...
### fn `def cmd_ll(extra)` (L4012-4024)
...
### fn `def cmd_lm(extra)` (L4029-4032)
...
### fn `def cmd_ls(extra)` (L4038-4041)
...
### fn `def cmd_lsi(extra)` (L4059-4079)
...
### fn `def cmd_lsa(extra)` (L4085-4088)
...
### fn `def cmd_lt(extra)` (L4094-4113)
...
### fn `def cmd_me(extra)` (L4118-4121)
...
### fn `def cmd_pl(extra)` (L4126-4129)
...
### fn `def cmd_pt(extra)` (L4134-4137)
...
### fn `def cmd_pu(extra)` (L4142-4145)
...
### fn `def cmd_rf(extra)` (L4150-4153)
...
### fn `def cmd_rmtg(extra)` (L4158-4168)
...
### fn `def cmd_rmloc(extra)` (L4173-4176)
...
### fn `def cmd_rmstg(extra)` (L4181-4184)
...
### fn `def cmd_rmunt(extra)` (L4189-4192)
...
### fn `def cmd_rs(extra)` (L4197-4200)
...
### fn `def cmd_rssft(extra)` (L4205-4208)
...
### fn `def cmd_rsmix(extra)` (L4213-4216)
...
### fn `def cmd_rshrd(extra)` (L4221-4224)
...
### fn `def cmd_rsmrg(extra)` (L4229-4232)
...
### fn `def cmd_rskep(extra)` (L4237-4240)
...
### fn `def cmd_st(extra)` (L4245-4248)
...
### fn `def cmd_tg(extra)` (L4253-4256)
...
### fn `def cmd_unstg(extra)` (L4261-4264)
...
### fn `def cmd_wt(extra)` (L4269-4272)
...
### fn `def cmd_wtl(extra)` (L4277-4280)
...
### fn `def cmd_wtp(extra)` (L4285-4288)
...
### fn `def cmd_wtr(extra)` (L4293-4296)
...
### fn `def cmd_ver(extra)` (L4301-4327)
...
### fn `def cmd_chver(extra)` (L4332-4415)
...
### fn `def cmd_major(extra)` (L4424-4428)
...
### fn `def cmd_minor(extra)` (L4437-4441)
...
### fn `def cmd_patch(extra)` (L4450-4454)
...
### fn `def cmd_backup(extra)` (L4462-4472)
...
### fn `def cmd_changelog(extra)` (L4482-4519)
...
### fn `def print_command_help(name, width=None)` (L4610-4617)
...
### fn `def print_all_help()` (L4621-4655)
...
### fn `def main(argv=None, *, check_updates: bool = True)` (L4661-4719)
...
|`VERSION_CHECK_RATE_LIMIT_IDLE_DELAY_SECONDS`|var|pub|71||
...
|`check_for_newer_version`|fn|pub|345-349|def check_for_newer_version(|
|`get_git_root`|fn|pub|469-484|def get_git_root()|
|`get_config_path`|fn|pub|489-493|def get_config_path(root=None)|
|`get_global_config_path`|fn|pub|498-502|def get_global_config_path(home=None)|
|`_read_config_object`|fn|priv|507-525|def _read_config_object(config_path)|
|`_apply_config_values`|fn|priv|531-553|def _apply_config_values(data, keys)|
|`load_cli_config`|fn|pub|559-571|def load_cli_config(root=None, home=None)|
|`_write_missing_config_values`|fn|priv|578-635|def _write_missing_config_values(config_path, keys, creat...|
|`write_default_config`|fn|pub|641-654|def write_default_config(root=None, home=None)|
|`_editor_base_command`|fn|priv|658-672|def _editor_base_command()|
|`run_editor_command`|fn|pub|677-680|def run_editor_command(args)|
|`_config_command_parts`|fn|priv|688-712|def _config_command_parts(key: str, default_command: str)...|
|`HELP_TEXTS`|var|pub|715||
|`RESET_HELP_COMMANDS`|var|pub|881||
|`LSI_DEFAULT_EXCLUDED_DIRS`|var|pub|887||
|`LSI_DEFAULT_EXCLUDED_DIR_SUFFIXES`|var|pub|924||
|`_to_args`|fn|priv|931-934|def _to_args(extra)|
|`CommandExecutionError`|class|pub|937-978|class CommandExecutionError(RuntimeError)|
|`CommandExecutionError.__init__`|fn|priv|942-949|def __init__(self, exc: subprocess.CalledProcessError)|
|`CommandExecutionError._format_message`|fn|priv|953-963|def _format_message(self) -> str|
|`CommandExecutionError._decode_stream`|fn|priv|968-978|def _decode_stream(data) -> str|
|`_run_checked`|fn|priv|984-991|def _run_checked(*popenargs, **kwargs)|
|`VersionDetectionError`|class|pub|994-997|class VersionDetectionError(RuntimeError)|
|`ReleaseError`|class|pub|1000-1003|class ReleaseError(RuntimeError)|
|`run_git_cmd`|fn|pub|1011-1015|def run_git_cmd(base_args, extra=None, cwd=None, **kwargs)|
|`capture_git_output`|fn|pub|1021-1027|def capture_git_output(base_args, cwd=None)|
|`run_command`|fn|pub|1033-1036|def run_command(cmd, cwd=None)|
|`run_git_text`|fn|pub|1043-1060|def run_git_text(args, cwd=None, check=True)|
|`run_shell`|fn|pub|1066-1069|def run_shell(command, cwd=None)|
|`_git_status_lines`|fn|priv|1073-1085|def _git_status_lines()|
|`has_unstaged_changes`|fn|pub|1090-1101|def has_unstaged_changes(status_lines=None)|
|`has_staged_changes`|fn|pub|1106-1115|def has_staged_changes(status_lines=None)|
|`WIP_MESSAGE_RE`|var|pub|1121||
|`_refresh_remote_refs`|fn|priv|1127-1138|def _refresh_remote_refs()|
|`_branch_remote_divergence`|fn|priv|1144-1164|def _branch_remote_divergence(branch_key, remote="origin")|
|`has_remote_branch_updates`|fn|pub|1170-1174|def has_remote_branch_updates(branch_key, remote="origin")|
|`has_remote_develop_updates`|fn|pub|1178-1181|def has_remote_develop_updates()|
|`has_remote_master_updates`|fn|pub|1185-1188|def has_remote_master_updates()|
|`_head_commit_message`|fn|priv|1192-1198|def _head_commit_message()|
|`_head_commit_hash`|fn|priv|1202-1208|def _head_commit_hash()|
|`_commit_exists_in_branch`|fn|priv|1214-1226|def _commit_exists_in_branch(commit_hash, branch_name)|
|`_should_amend_existing_commit`|fn|priv|1230-1245|def _should_amend_existing_commit()|
|`is_inside_git_repo`|fn|pub|1249-1256|def is_inside_git_repo()|
|`TagInfo`|class|pub|1261-1269|class TagInfo|
|`DELIM`|var|pub|1272||
|`RECORD`|var|pub|1275||
|`SEMVER_RE`|var|pub|1291||
|`SECTION_EMOJI`|var|pub|1294||
|`_tag_semver_tuple`|fn|priv|1312-1315|def _tag_semver_tuple(tag_name: str) -> Optional[Tuple[in...|
|`_latest_supported_tag_name`|fn|priv|1320-1323|def _latest_supported_tag_name(tags: List[TagInfo]) -> Op...|
|`_is_minor_release_tag`|fn|priv|1331-1338|def _is_minor_release_tag(tag_name: str) -> bool|
|`_latest_patch_tag_after`|fn|priv|1347-1348|def _latest_patch_tag_after(|
|`list_tags_sorted_by_date`|fn|pub|1366-1367|def list_tags_sorted_by_date(|
|`git_log_subjects`|fn|pub|1397-1408|def git_log_subjects(repo_root: Path, rev_range: str) -> ...|
|`parse_conventional_commit`|fn|pub|1414-1415|def parse_conventional_commit(|
|`_format_changelog_description`|fn|priv|1435-1448|def _format_changelog_description(desc: str) -> List[str]|
|`categorize_commit`|fn|pub|1456-1485|def categorize_commit(subject: str) -> Tuple[Optional[str...|
|`_extract_release_version`|fn|priv|1490-1500|def _extract_release_version(subject: str) -> Optional[str]|
|`_is_release_marker_commit`|fn|priv|1505-1508|def _is_release_marker_commit(subject: str) -> bool|
|`generate_section_for_range`|fn|pub|1517-1522|def generate_section_for_range(|
|`_get_remote_name_for_branch`|fn|priv|1570-1578|def _get_remote_name_for_branch(branch_name: str, repo_ro...|
|`_extract_owner_repo`|fn|priv|1586-1610|def _extract_owner_repo(remote_url: str) -> Optional[Tupl...|
|`_canonical_origin_base`|fn|priv|1620-1635|def _canonical_origin_base(repo_root: Path) -> Optional[str]|
|`get_origin_compare_url`|fn|pub|1642-1643|def get_origin_compare_url(|
|`get_release_page_url`|fn|pub|1657-1662|def get_release_page_url(base_url: Optional[str], tag: st...|
|`build_history_section`|fn|pub|1670-1674|def build_history_section(|
|`generate_changelog_document`|fn|pub|1724-1725|def generate_changelog_document(|
|`VersionRuleContext`|class|pub|1797-1804|class VersionRuleContext|
|`_normalize_version_rule_pattern`|fn|priv|1810-1821|def _normalize_version_rule_pattern(pattern: str) -> str|
|`_build_version_file_inventory`|fn|priv|1827-1854|def _build_version_file_inventory(root: Path) -> List[Tup...|
|`_collect_version_files`|fn|priv|1862-1885|def _collect_version_files(root, pattern, *, inventory=None)|
|`_is_version_path_excluded`|fn|priv|1890-1893|def _is_version_path_excluded(relative_path: str) -> bool|
|`_iter_versions_in_text`|fn|priv|1899-1910|def _iter_versions_in_text(text, compiled_regexes)|
|`_read_version_file_text`|fn|priv|1917-1918|def _read_version_file_text(|
|`_prepare_version_rule_contexts`|fn|priv|1942-1943|def _prepare_version_rule_contexts(|
|`_determine_canonical_version`|fn|priv|1986-1993|def _determine_canonical_version(|
|`_parse_semver_tuple`|fn|priv|2046-2052|def _parse_semver_tuple(text: str) -> Optional[Tuple[int,...|
|`_replace_versions_in_text`|fn|priv|2059-2074|def _replace_versions_in_text(text, compiled_regex, repla...|
|`_current_branch_name`|fn|priv|2078-2090|def _current_branch_name()|
|`_ref_exists`|fn|priv|2095-2104|def _ref_exists(ref_name)|
|`_local_branch_exists`|fn|priv|2109-2112|def _local_branch_exists(branch_name)|
|`_remote_branch_exists`|fn|priv|2117-2120|def _remote_branch_exists(branch_name)|
|`_ensure_release_prerequisites`|fn|priv|2124-2173|def _ensure_release_prerequisites()|
|`_bump_semver_version`|fn|priv|2179-2199|def _bump_semver_version(current_version, level)|
|`_run_release_step`|fn|priv|2206-2232|def _run_release_step(level, step_name, action)|
|`_create_release_commit_for_flow`|fn|priv|2237-2242|def _create_release_commit_for_flow(target_version)|
|`_push_branch_with_tags`|fn|priv|2248-2252|def _push_branch_with_tags(branch_name)|
|`_execute_release_flow`|fn|priv|2273-2364|def _execute_release_flow(level, changelog_args=None)|
|`_execute_backup_flow`|fn|priv|2372-2389|def _execute_backup_flow()|
|`_run_release_command`|fn|priv|2395-2410|def _run_release_command(level, changelog_args=None)|
|`_run_backup_command`|fn|priv|2415-2422|def _run_backup_command()|
|`_run_reset_with_help`|fn|priv|2428-2435|def _run_reset_with_help(base_args, extra)|
|`_reject_extra_arguments`|fn|priv|2441-2447|def _reject_extra_arguments(extra, alias)|
|`_parse_release_flags`|fn|priv|2453-2473|def _parse_release_flags(extra, alias)|
|`_prepare_commit_message`|fn|priv|2479-2489|def _prepare_commit_message(extra, alias)|
|`_normalize_conventional_description`|fn|priv|2496-2506|def _normalize_conventional_description(description: str)...|
|`_build_conventional_message`|fn|priv|2515-2536|def _build_conventional_message(kind: str, extra, alias: ...|
|`_run_conventional_commit`|fn|priv|2554-2559|def _run_conventional_commit(kind: str, alias: str, extra)|
|`_execute_commit`|fn|priv|2566-2598|def _execute_commit(message, alias, allow_amend=True)|
|`_ensure_clean_worktree_and_index`|fn|priv|2606-2622|def _ensure_clean_worktree_and_index(alias: str) -> bool|
|`_resolve_rollback_target_commit`|fn|priv|2630-2640|def _resolve_rollback_target_commit(target: str) -> str|
|`_is_commit_ancestor`|fn|priv|2647-2656|def _is_commit_ancestor(ancestor_commit: str, descendant_...|
|`_is_commit_hash_token`|fn|priv|2661-2664|def _is_commit_hash_token(token: str) -> bool|
|`_tag_exists`|fn|priv|2669-2672|def _tag_exists(tag_name: str) -> bool|
|`_is_linux_platform`|fn|priv|2677-2680|def _is_linux_platform() -> bool|
|`_print_non_linux_management_command`|fn|priv|2687-2689|def _print_non_linux_management_command(|
|`upgrade_self`|fn|pub|2703-2710|def upgrade_self(repo_root: Optional[Path] = None)|
|`_remove_version_check_cache_artifacts`|fn|priv|2717-2738|def _remove_version_check_cache_artifacts() -> None|
|`uninstall_self`|fn|pub|2743-2750|def uninstall_self()|
|`cmd_aa`|fn|pub|2755-2762|def cmd_aa(extra)|
|`cmd_ra`|fn|pub|2767-2790|def cmd_ra(extra)|
|`cmd_ar`|fn|pub|2795-2811|def cmd_ar(extra)|
|`cmd_br`|fn|pub|2816-2819|def cmd_br(extra)|
|`cmd_bd`|fn|pub|2824-2827|def cmd_bd(extra)|
|`cmd_ck`|fn|pub|2832-2835|def cmd_ck(extra)|
|`_ensure_commit_ready`|fn|priv|2840-2853|def _ensure_commit_ready(alias)|
|`_ensure_commit_ready_with_stage`|fn|priv|2871-2885|def _ensure_commit_ready_with_stage(alias)|
|`cmd_cm`|fn|pub|2890-2895|def cmd_cm(extra)|
|`cmd_wip`|fn|pub|2905-2917|def cmd_wip(extra)|
|`cmd_release`|fn|pub|2922-2944|def cmd_release(extra)|
|`cmd_new`|fn|pub|2949-2952|def cmd_new(extra)|
|`cmd_refactor`|fn|pub|2957-2960|def cmd_refactor(extra)|
|`cmd_fix`|fn|pub|2965-2968|def cmd_fix(extra)|
|`cmd_change`|fn|pub|2973-2976|def cmd_change(extra)|
|`cmd_implement`|fn|pub|2981-2984|def cmd_implement(extra)|
|`cmd_docs`|fn|pub|2989-2992|def cmd_docs(extra)|
|`cmd_style`|fn|pub|2997-3000|def cmd_style(extra)|
|`cmd_revert`|fn|pub|3005-3008|def cmd_revert(extra)|
|`cmd_rollback`|fn|pub|3017-3070|def cmd_rollback(extra)|
|`cmd_misc`|fn|pub|3075-3078|def cmd_misc(extra)|
|`cmd_cover`|fn|pub|3083-3086|def cmd_cover(extra)|
|`cmd_co`|fn|pub|3091-3094|def cmd_co(extra)|
|`cmd_dc`|fn|pub|3099-3108|def cmd_dc(extra)|
|`cmd_dcc`|fn|pub|3113-3116|def cmd_dcc(extra)|
|`cmd_dccc`|fn|pub|3121-3124|def cmd_dccc(extra)|
|`cmd_de`|fn|pub|3129-3132|def cmd_de(extra)|
|`cmd_di`|fn|pub|3137-3140|def cmd_di(extra)|
|`cmd_diyou`|fn|pub|3145-3148|def cmd_diyou(extra)|
|`cmd_dime`|fn|pub|3153-3156|def cmd_dime(extra)|
|`cmd_dwc`|fn|pub|3161-3164|def cmd_dwc(extra)|
|`cmd_dw`|fn|pub|3169-3176|def cmd_dw(extra)|
|`cmd_dwcc`|fn|pub|3181-3184|def cmd_dwcc(extra)|
|`cmd_dcd`|fn|pub|3190-3195|def cmd_dcd(extra)|
|`cmd_dcm`|fn|pub|3201-3206|def cmd_dcm(extra)|
|`cmd_ddm`|fn|pub|3212-3217|def cmd_ddm(extra)|
|`cmd_ed`|fn|pub|3222-3231|def cmd_ed(extra)|
|`cmd_fe`|fn|pub|3236-3239|def cmd_fe(extra)|
|`cmd_feall`|fn|pub|3244-3247|def cmd_feall(extra)|
|`cmd_gp`|fn|pub|3252-3257|def cmd_gp(extra)|
|`cmd_gr`|fn|pub|3262-3267|def cmd_gr(extra)|
|`OVERVIEW_COLOR_RESET`|var|pub|3269||
|`OVERVIEW_COLOR_SECTION_PURPLE`|var|pub|3271||
|`OVERVIEW_COLOR_AHEAD`|var|pub|3273||
|`OVERVIEW_COLOR_BEHIND`|var|pub|3275||
|`OVERVIEW_COLOR_LABEL`|var|pub|3277||
|`OVERVIEW_COLOR_WHITE`|var|pub|3279||
|`OVERVIEW_COLOR_WHITE_BOLD`|var|pub|3281||
|`OVERVIEW_SECTION_TEMPLATE`|var|pub|3283||
|`OVERVIEW_SUBSECTION_TEMPLATE`|var|pub|3285||
|`OVERVIEW_DISTANCE_TEMPLATE`|var|pub|3287||
|`_overview_branch_identifier`|fn|priv|3296-3299|def _overview_branch_identifier(|
|`_overview_work_prefix_color`|fn|priv|3313-3320|def _overview_work_prefix_color(worktree_state: str) -> str|
|`_overview_logical_branch_name`|fn|priv|3328-3332|def _overview_logical_branch_name(|
|`_overview_current_branch_display`|fn|priv|3351-3356|def _overview_current_branch_display(|
|`_overview_ref_is_available`|fn|priv|3378-3387|def _overview_ref_is_available(ref_name: str) -> bool|
|`_overview_ref_latest_subject`|fn|priv|3393-3403|def _overview_ref_latest_subject(ref_name: str) -> str|
|`_overview_discovered_branch_refs`|fn|priv|3408-3431|def _overview_discovered_branch_refs() -> List[str]|
|`_overview_branch_summary_lines`|fn|priv|3464-3475|def _overview_branch_summary_lines(|
|`_overview_relation_state`|fn|priv|3518-3527|def _overview_relation_state(ahead: int, behind: int) -> str|
|`_overview_worktree_state`|fn|priv|3532-3544|def _overview_worktree_state(status_lines=None) -> str|
|`_overview_distance_text`|fn|priv|3550-3558|def _overview_distance_text(is_ahead: bool, count: int) -...|
|`_overview_compare_refs`|fn|priv|3565-3607|def _overview_compare_refs(base_ref: str, target_ref: str...|
|`_overview_ascii_topology_lines`|fn|priv|3642-3653|def _overview_ascii_topology_lines(|
|`_overview_current_branch_state_lines`|fn|priv|3745-3764|def _overview_current_branch_state_lines(current_branch_d...|
|`cmd_o`|fn|pub|3770-3924|def cmd_o(extra)|
|`cmd_str`|fn|pub|3929-3958|def cmd_str(extra)|
|`cmd_l`|fn|pub|3967-3973|def cmd_l(extra)|
|`cmd_lb`|fn|pub|3978-3981|def cmd_lb(extra)|
|`cmd_lg`|fn|pub|3986-3999|def cmd_lg(extra)|
|`cmd_lh`|fn|pub|4004-4007|def cmd_lh(extra)|
|`cmd_ll`|fn|pub|4012-4024|def cmd_ll(extra)|
|`cmd_lm`|fn|pub|4029-4032|def cmd_lm(extra)|
|`cmd_ls`|fn|pub|4038-4041|def cmd_ls(extra)|
|`cmd_lsi`|fn|pub|4059-4079|def cmd_lsi(extra)|
|`cmd_lsa`|fn|pub|4085-4088|def cmd_lsa(extra)|
|`cmd_lt`|fn|pub|4094-4113|def cmd_lt(extra)|
|`cmd_me`|fn|pub|4118-4121|def cmd_me(extra)|
|`cmd_pl`|fn|pub|4126-4129|def cmd_pl(extra)|
|`cmd_pt`|fn|pub|4134-4137|def cmd_pt(extra)|
|`cmd_pu`|fn|pub|4142-4145|def cmd_pu(extra)|
|`cmd_rf`|fn|pub|4150-4153|def cmd_rf(extra)|
|`cmd_rmtg`|fn|pub|4158-4168|def cmd_rmtg(extra)|
|`cmd_rmloc`|fn|pub|4173-4176|def cmd_rmloc(extra)|
|`cmd_rmstg`|fn|pub|4181-4184|def cmd_rmstg(extra)|
|`cmd_rmunt`|fn|pub|4189-4192|def cmd_rmunt(extra)|
|`cmd_rs`|fn|pub|4197-4200|def cmd_rs(extra)|
|`cmd_rssft`|fn|pub|4205-4208|def cmd_rssft(extra)|
|`cmd_rsmix`|fn|pub|4213-4216|def cmd_rsmix(extra)|
|`cmd_rshrd`|fn|pub|4221-4224|def cmd_rshrd(extra)|
|`cmd_rsmrg`|fn|pub|4229-4232|def cmd_rsmrg(extra)|
|`cmd_rskep`|fn|pub|4237-4240|def cmd_rskep(extra)|
|`cmd_st`|fn|pub|4245-4248|def cmd_st(extra)|
|`cmd_tg`|fn|pub|4253-4256|def cmd_tg(extra)|
|`cmd_unstg`|fn|pub|4261-4264|def cmd_unstg(extra)|
|`cmd_wt`|fn|pub|4269-4272|def cmd_wt(extra)|
|`cmd_wtl`|fn|pub|4277-4280|def cmd_wtl(extra)|
|`cmd_wtp`|fn|pub|4285-4288|def cmd_wtp(extra)|
|`cmd_wtr`|fn|pub|4293-4296|def cmd_wtr(extra)|
|`cmd_ver`|fn|pub|4301-4327|def cmd_ver(extra)|
|`cmd_chver`|fn|pub|4332-4415|def cmd_chver(extra)|
|`cmd_major`|fn|pub|4424-4428|def cmd_major(extra)|
|`cmd_minor`|fn|pub|4437-4441|def cmd_minor(extra)|
|`cmd_patch`|fn|pub|4450-4454|def cmd_patch(extra)|
|`cmd_backup`|fn|pub|4462-4472|def cmd_backup(extra)|
|`cmd_changelog`|fn|pub|4482-4519|def cmd_changelog(extra)|
|`COMMANDS`|var|pub|4522||
|`print_command_help`|fn|pub|4610-4617|def print_command_help(name, width=None)|
|`print_all_help`|fn|pub|4621-4655|def print_all_help()|
|`main`|fn|pub|4661-4719|def main(argv=None, *, check_updates: bool = True)|
