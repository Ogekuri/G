...
# core.py | Python | 3858L | 225 symbols | 17 imports | 986 comments
...
- var `RESET_HELP_COMMANDS = {"rs", "rshrd", "rskep", "rsmix", "rsmrg", "rssft"}` (L674)
- Brief: Constant `RESET_HELP_COMMANDS` used by CLI runtime paths and policies.
### fn `def _to_args(extra)` `priv` (L681-684)
...
### class `class CommandExecutionError(RuntimeError)` : RuntimeError (L686-727)
...
- fn `def __init__(self, exc: subprocess.CalledProcessError)` `priv` (L691-698)
  - Brief: Execute `__init__` runtime logic for Git-Alias CLI.
  - Param: self Input parameter consumed by `__init__`.
  - Param: exc Input parameter consumed by `__init__`.
  - Return: Result emitted by `__init__` according to command contract.
- fn `def _format_message(self) -> str` `priv` (L702-712)
...
- fn `def _decode_stream(data) -> str` `priv` (L717-727)
...
### fn `def _run_checked(*popenargs, **kwargs)` `priv` (L733-740)
...
### class `class VersionDetectionError(RuntimeError)` : RuntimeError (L742-745)
- Brief: Class `VersionDetectionError` models a typed runtime container/error boundary.

### class `class ReleaseError(RuntimeError)` : RuntimeError (L747-750)
- Brief: Class `ReleaseError` models a typed runtime container/error boundary.

### fn `def run_git_cmd(base_args, extra=None, cwd=None, **kwargs)` (L758-762)
...
### fn `def capture_git_output(base_args, cwd=None)` (L768-772)
...
### fn `def run_command(cmd, cwd=None)` (L778-781)
...
### fn `def run_git_text(args, cwd=None, check=True)` (L788-805)
...
### fn `def run_shell(command, cwd=None)` (L811-814)
...
### fn `def _git_status_lines()` `priv` (L818-830)
...
### fn `def has_unstaged_changes(status_lines=None)` (L835-846)
...
### fn `def has_staged_changes(status_lines=None)` (L851-860)
...
- var `WIP_MESSAGE_RE = re.compile(r"^wip: work in progress\.$")` (L866)
- Brief: Constant `WIP_MESSAGE_RE` used by CLI runtime paths and policies.
### fn `def _refresh_remote_refs()` `priv` (L872-883)
...
### fn `def _branch_remote_divergence(branch_key, remote="origin")` `priv` (L889-907)
...
### fn `def has_remote_branch_updates(branch_key, remote="origin")` (L913-917)
...
### fn `def has_remote_develop_updates()` (L921-924)
...
### fn `def has_remote_master_updates()` (L928-931)
...
### fn `def _head_commit_message()` `priv` (L935-941)
...
### fn `def _head_commit_hash()` `priv` (L945-951)
...
### fn `def _commit_exists_in_branch(commit_hash, branch_name)` `priv` (L957-969)
...
### fn `def _should_amend_existing_commit()` `priv` (L973-988)
...
### fn `def is_inside_git_repo()` (L992-999)
...
### class `class TagInfo` (L1004-1012)
...
### fn `def _latest_supported_tag_name(tags: List[TagInfo]) -> Optional[str]` `priv` (L1062-1065)
...
### fn `def _is_minor_release_tag(tag_name: str) -> bool` `priv` (L1073-1080)
...
### fn `def _latest_patch_tag_after(all_tags: List[TagInfo], last_minor: Optional[TagInfo]) -> Optional[TagInfo]` `priv` (L1089-1098)
...
### fn `def list_tags_sorted_by_date(repo_root: Path, merged_ref: Optional[str] = None) -> List[TagInfo]` (L1104-1124)
...
### fn `def git_log_subjects(repo_root: Path, rev_range: str) -> List[str]` (L1131-1142)
...
### fn `def parse_conventional_commit(message: str) -> Optional[Tuple[str, Optional[str], bool, str]]` (L1148-1160)
...
### fn `def _format_changelog_description(desc: str) -> List[str]` `priv` (L1167-1178)
...
### fn `def categorize_commit(subject: str) -> Tuple[Optional[str], str]` (L1186-1215)
...
### fn `def _extract_release_version(subject: str) -> Optional[str]` `priv` (L1220-1230)
...
### fn `def _is_release_marker_commit(subject: str) -> bool` `priv` (L1235-1238)
...
### fn `def generate_section_for_range(repo_root: Path, title: str, date_s: str, rev_range: str, expected_version: Optional[str] = None) -> Optional[str]` (L1247-1285)
...
### fn `def _get_remote_name_for_branch(branch_name: str, repo_root: Path) -> str` `priv` (L1294-1302)
...
### fn `def _extract_owner_repo(remote_url: str) -> Optional[Tuple[str, str]]` `priv` (L1309-1333)
...
### fn `def _canonical_origin_base(repo_root: Path) -> Optional[str]` `priv` (L1343-1356)
...
### fn `def get_origin_compare_url(base_url: Optional[str], prev_tag: Optional[str], tag: str) -> Optional[str]` (L1363-1370)
...
### fn `def get_release_page_url(base_url: Optional[str], tag: str) -> Optional[str]` (L1376-1381)
...
### fn `def build_history_section(` (L1389-1393)
...
### fn `def generate_changelog_document(repo_root: Path, include_patch: bool, disable_history: bool = False) -> str` (L1437-1498)
...
### class `class VersionRuleContext` `@dataclass(frozen=True)` (L1509-1516)

### fn `def _normalize_version_rule_pattern(pattern: str) -> str` `priv` (L1522-1533)
...
### fn `def _build_version_file_inventory(root: Path) -> List[Tuple[Path, str]]` `priv` (L1539-1560)
...
### fn `def _collect_version_files(root, pattern, *, inventory=None)` `priv` (L1568-1585)
...
### fn `def _is_version_path_excluded(relative_path: str) -> bool` `priv` (L1590-1593)
...
### fn `def _iter_versions_in_text(text, compiled_regexes)` `priv` (L1599-1610)
...
### fn `def _read_version_file_text(file_path: Path, text_cache: Optional[Dict[Path, str]] = None) -> Optional[str]` `priv` (L1617-1631)
...
### fn `def _prepare_version_rule_contexts(` `priv` (L1640-1641)
...
### fn `def _determine_canonical_version(` `priv` (L1683-1690)
...
### fn `def _parse_semver_tuple(text: str) -> Optional[Tuple[int, int, int]]` `priv` (L1735-1741)
...
### fn `def _replace_versions_in_text(text, compiled_regex, replacement)` `priv` (L1748-1763)
...
### fn `def _current_branch_name()` `priv` (L1767-1779)
...
### fn `def _ref_exists(ref_name)` `priv` (L1784-1793)
...
### fn `def _local_branch_exists(branch_name)` `priv` (L1798-1801)
...
### fn `def _remote_branch_exists(branch_name)` `priv` (L1806-1809)
...
### fn `def _ensure_release_prerequisites()` `priv` (L1813-1840)
...
### fn `def _bump_semver_version(current_version, level)` `priv` (L1846-1864)
...
### fn `def _run_release_step(level, step_name, action)` `priv` (L1871-1891)
...
### fn `def _create_release_commit_for_flow(target_version)` `priv` (L1896-1901)
...
### fn `def _push_branch_with_tags(branch_name)` `priv` (L1907-1911)
...
### fn `def _execute_release_flow(level, changelog_args=None)` `priv` (L1927-1974)
...
### fn `def _execute_backup_flow()` `priv` (L1982-1997)
...
### fn `def _run_release_command(level, changelog_args=None)` `priv` (L2003-2018)
...
### fn `def _run_backup_command()` `priv` (L2023-2030)
...
### fn `def _run_reset_with_help(base_args, extra)` `priv` (L2036-2043)
...
### fn `def _reject_extra_arguments(extra, alias)` `priv` (L2049-2055)
...
### fn `def _parse_release_flags(extra, alias)` `priv` (L2061-2079)
...
### fn `def _prepare_commit_message(extra, alias)` `priv` (L2085-2095)
...
### fn `def _normalize_conventional_description(description: str) -> str` `priv` (L2102-2112)
...
### fn `def _build_conventional_message(kind: str, extra, alias: str) -> str` `priv` (L2119-2134)
...
### fn `def _run_conventional_commit(kind: str, alias: str, extra)` `priv` (L2141-2146)
...
### fn `def _execute_commit(message, alias, allow_amend=True)` `priv` (L2153-2182)
...
### fn `def upgrade_self()` (L2186-2199)
...
### fn `def cmd_aa(extra)` (L2211-2218)
...
### fn `def cmd_ra(extra)` (L2223-2246)
...
### fn `def cmd_ar(extra)` (L2251-2265)
...
### fn `def cmd_br(extra)` (L2270-2273)
...
### fn `def cmd_bd(extra)` (L2278-2281)
...
### fn `def cmd_ck(extra)` (L2286-2289)
...
### fn `def _ensure_commit_ready(alias)` `priv` (L2294-2307)
...
### fn `def cmd_cm(extra)` (L2312-2317)
...
### fn `def cmd_wip(extra)` (L2322-2334)
...
### fn `def cmd_release(extra)` (L2339-2361)
...
### fn `def cmd_new(extra)` (L2366-2369)
...
### fn `def cmd_refactor(extra)` (L2374-2377)
...
### fn `def cmd_fix(extra)` (L2382-2385)
...
### fn `def cmd_change(extra)` (L2390-2393)
...
### fn `def cmd_implement(extra)` (L2398-2401)
...
### fn `def cmd_docs(extra)` (L2406-2409)
...
### fn `def cmd_style(extra)` (L2414-2417)
...
### fn `def cmd_revert(extra)` (L2422-2425)
...
### fn `def cmd_misc(extra)` (L2430-2433)
...
### fn `def cmd_cover(extra)` (L2438-2441)
...
### fn `def cmd_co(extra)` (L2446-2449)
...
### fn `def cmd_dr(extra)` (L2454-2461)
...
### fn `def cmd_dcc(extra)` (L2466-2469)
...
### fn `def cmd_dccc(extra)` (L2474-2477)
...
### fn `def cmd_de(extra)` (L2482-2485)
...
### fn `def cmd_di(extra)` (L2490-2493)
...
### fn `def cmd_diyou(extra)` (L2498-2501)
...
### fn `def cmd_dime(extra)` (L2506-2509)
...
### fn `def cmd_dwc(extra)` (L2514-2517)
...
### fn `def cmd_dwcc(extra)` (L2522-2525)
...
### fn `def cmd_ed(extra)` (L2530-2539)
...
### fn `def cmd_fe(extra)` (L2544-2547)
...
### fn `def cmd_feall(extra)` (L2552-2555)
...
### fn `def cmd_gp(extra)` (L2560-2563)
...
### fn `def cmd_gr(extra)` (L2568-2571)
...
### fn `def _overview_work_prefix_color(worktree_state: str) -> str` `priv` (L2617-2624)
...
### fn `def _overview_logical_branch_name(` `priv` (L2632-2636)
...
### fn `def _overview_current_branch_display(` `priv` (L2655-2660)
...
### fn `def _overview_ref_is_available(ref_name: str) -> bool` `priv` (L2682-2691)
...
### fn `def _overview_ref_latest_subject(ref_name: str) -> str` `priv` (L2697-2706)
...
### fn `def _overview_branch_summary_lines(` `priv` (L2723-2733)
...
### fn `def _overview_relation_state(ahead: int, behind: int) -> str` `priv` (L2761-2770)
...
### fn `def _overview_worktree_state(status_lines=None) -> str` `priv` (L2775-2787)
...
### fn `def _overview_distance_text(is_ahead: bool, count: int) -> str` `priv` (L2793-2801)
...
### fn `def _overview_compare_refs(base_ref: str, target_ref: str, label: str) -> str` `priv` (L2808-2848)
...
### fn `def _overview_ascii_topology_lines(` `priv` (L2869-2880)
...
### fn `def _overview_current_branch_state_lines(current_branch_display: str) -> List[str]` `priv` (L2970-2989)
...
### fn `def cmd_str(extra)` (L3141-3170)
...
### fn `def cmd_lb(extra)` (L3175-3178)
...
### fn `def cmd_lg(extra)` (L3183-3196)
...
### fn `def cmd_lh(extra)` (L3201-3204)
...
### fn `def cmd_ll(extra)` (L3209-3221)
...
### fn `def cmd_lm(extra)` (L3226-3229)
...
### fn `def cmd_ls(extra)` (L3235-3238)
...
### fn `def cmd_lsi(extra)` (L3244-3250)
...
### fn `def cmd_lsa(extra)` (L3256-3259)
...
### fn `def cmd_lt(extra)` (L3265-3284)
...
### fn `def cmd_me(extra)` (L3289-3292)
...
### fn `def cmd_pl(extra)` (L3297-3300)
...
### fn `def cmd_pt(extra)` (L3305-3308)
...
### fn `def cmd_pu(extra)` (L3313-3316)
...
### fn `def cmd_rf(extra)` (L3321-3324)
...
### fn `def cmd_rmtg(extra)` (L3329-3339)
...
### fn `def cmd_rmloc(extra)` (L3344-3347)
...
### fn `def cmd_rmstg(extra)` (L3352-3355)
...
### fn `def cmd_rmunt(extra)` (L3360-3363)
...
### fn `def cmd_rs(extra)` (L3368-3371)
...
### fn `def cmd_rssft(extra)` (L3376-3379)
...
### fn `def cmd_rsmix(extra)` (L3384-3387)
...
### fn `def cmd_rshrd(extra)` (L3392-3395)
...
### fn `def cmd_rsmrg(extra)` (L3400-3403)
...
### fn `def cmd_rskep(extra)` (L3408-3411)
...
### fn `def cmd_st(extra)` (L3416-3419)
...
### fn `def cmd_tg(extra)` (L3424-3427)
...
### fn `def cmd_unstg(extra)` (L3432-3435)
...
### fn `def cmd_wt(extra)` (L3440-3443)
...
### fn `def cmd_wtl(extra)` (L3448-3451)
...
### fn `def cmd_wtp(extra)` (L3456-3459)
...
### fn `def cmd_wtr(extra)` (L3464-3467)
...
### fn `def cmd_ver(extra)` (L3472-3498)
...
### fn `def cmd_chver(extra)` (L3503-3575)
...
### fn `def cmd_major(extra)` (L3584-3588)
...
### fn `def cmd_minor(extra)` (L3597-3601)
...
### fn `def cmd_patch(extra)` (L3610-3614)
...
### fn `def cmd_backup(extra)` (L3622-3632)
...
### fn `def cmd_changelog(extra)` (L3642-3674)
...
### fn `def print_command_help(name, width=None)` (L3758-3764)
...
### fn `def print_all_help()` (L3768-3802)
...
### fn `def main(argv=None, *, check_updates: bool = True)` (L3808-3858)
...
|`RESET_HELP_COMMANDS`|var|pub|674||
|`_to_args`|fn|priv|681-684|def _to_args(extra)|
|`CommandExecutionError`|class|pub|686-727|class CommandExecutionError(RuntimeError)|
|`CommandExecutionError.__init__`|fn|priv|691-698|def __init__(self, exc: subprocess.CalledProcessError)|
|`CommandExecutionError._format_message`|fn|priv|702-712|def _format_message(self) -> str|
|`CommandExecutionError._decode_stream`|fn|priv|717-727|def _decode_stream(data) -> str|
|`_run_checked`|fn|priv|733-740|def _run_checked(*popenargs, **kwargs)|
|`VersionDetectionError`|class|pub|742-745|class VersionDetectionError(RuntimeError)|
|`ReleaseError`|class|pub|747-750|class ReleaseError(RuntimeError)|
|`run_git_cmd`|fn|pub|758-762|def run_git_cmd(base_args, extra=None, cwd=None, **kwargs)|
|`capture_git_output`|fn|pub|768-772|def capture_git_output(base_args, cwd=None)|
|`run_command`|fn|pub|778-781|def run_command(cmd, cwd=None)|
|`run_git_text`|fn|pub|788-805|def run_git_text(args, cwd=None, check=True)|
|`run_shell`|fn|pub|811-814|def run_shell(command, cwd=None)|
|`_git_status_lines`|fn|priv|818-830|def _git_status_lines()|
|`has_unstaged_changes`|fn|pub|835-846|def has_unstaged_changes(status_lines=None)|
|`has_staged_changes`|fn|pub|851-860|def has_staged_changes(status_lines=None)|
|`WIP_MESSAGE_RE`|var|pub|866||
|`_refresh_remote_refs`|fn|priv|872-883|def _refresh_remote_refs()|
|`_branch_remote_divergence`|fn|priv|889-907|def _branch_remote_divergence(branch_key, remote="origin")|
|`has_remote_branch_updates`|fn|pub|913-917|def has_remote_branch_updates(branch_key, remote="origin")|
|`has_remote_develop_updates`|fn|pub|921-924|def has_remote_develop_updates()|
|`has_remote_master_updates`|fn|pub|928-931|def has_remote_master_updates()|
|`_head_commit_message`|fn|priv|935-941|def _head_commit_message()|
|`_head_commit_hash`|fn|priv|945-951|def _head_commit_hash()|
|`_commit_exists_in_branch`|fn|priv|957-969|def _commit_exists_in_branch(commit_hash, branch_name)|
|`_should_amend_existing_commit`|fn|priv|973-988|def _should_amend_existing_commit()|
|`is_inside_git_repo`|fn|pub|992-999|def is_inside_git_repo()|
|`TagInfo`|class|pub|1004-1012|class TagInfo|
|`DELIM`|var|pub|1015||
|`RECORD`|var|pub|1018||
|`SEMVER_RE`|var|pub|1034||
|`SECTION_EMOJI`|var|pub|1037||
|`_tag_semver_tuple`|fn|priv|1054-1057|def _tag_semver_tuple(tag_name: str) -> Optional[Tuple[in...|
|`_latest_supported_tag_name`|fn|priv|1062-1065|def _latest_supported_tag_name(tags: List[TagInfo]) -> Op...|
|`_is_minor_release_tag`|fn|priv|1073-1080|def _is_minor_release_tag(tag_name: str) -> bool|
|`_latest_patch_tag_after`|fn|priv|1089-1098|def _latest_patch_tag_after(all_tags: List[TagInfo], last...|
|`list_tags_sorted_by_date`|fn|pub|1104-1124|def list_tags_sorted_by_date(repo_root: Path, merged_ref:...|
|`git_log_subjects`|fn|pub|1131-1142|def git_log_subjects(repo_root: Path, rev_range: str) -> ...|
|`parse_conventional_commit`|fn|pub|1148-1160|def parse_conventional_commit(message: str) -> Optional[T...|
|`_format_changelog_description`|fn|priv|1167-1178|def _format_changelog_description(desc: str) -> List[str]|
|`categorize_commit`|fn|pub|1186-1215|def categorize_commit(subject: str) -> Tuple[Optional[str...|
|`_extract_release_version`|fn|priv|1220-1230|def _extract_release_version(subject: str) -> Optional[str]|
|`_is_release_marker_commit`|fn|priv|1235-1238|def _is_release_marker_commit(subject: str) -> bool|
|`generate_section_for_range`|fn|pub|1247-1285|def generate_section_for_range(repo_root: Path, title: st...|
|`_get_remote_name_for_branch`|fn|priv|1294-1302|def _get_remote_name_for_branch(branch_name: str, repo_ro...|
|`_extract_owner_repo`|fn|priv|1309-1333|def _extract_owner_repo(remote_url: str) -> Optional[Tupl...|
|`_canonical_origin_base`|fn|priv|1343-1356|def _canonical_origin_base(repo_root: Path) -> Optional[str]|
|`get_origin_compare_url`|fn|pub|1363-1370|def get_origin_compare_url(base_url: Optional[str], prev_...|
|`get_release_page_url`|fn|pub|1376-1381|def get_release_page_url(base_url: Optional[str], tag: st...|
|`build_history_section`|fn|pub|1389-1393|def build_history_section(|
|`generate_changelog_document`|fn|pub|1437-1498|def generate_changelog_document(repo_root: Path, include_...|
|`VersionRuleContext`|class|pub|1509-1516|class VersionRuleContext|
|`_normalize_version_rule_pattern`|fn|priv|1522-1533|def _normalize_version_rule_pattern(pattern: str) -> str|
|`_build_version_file_inventory`|fn|priv|1539-1560|def _build_version_file_inventory(root: Path) -> List[Tup...|
|`_collect_version_files`|fn|priv|1568-1585|def _collect_version_files(root, pattern, *, inventory=None)|
|`_is_version_path_excluded`|fn|priv|1590-1593|def _is_version_path_excluded(relative_path: str) -> bool|
|`_iter_versions_in_text`|fn|priv|1599-1610|def _iter_versions_in_text(text, compiled_regexes)|
|`_read_version_file_text`|fn|priv|1617-1631|def _read_version_file_text(file_path: Path, text_cache: ...|
|`_prepare_version_rule_contexts`|fn|priv|1640-1641|def _prepare_version_rule_contexts(|
|`_determine_canonical_version`|fn|priv|1683-1690|def _determine_canonical_version(|
|`_parse_semver_tuple`|fn|priv|1735-1741|def _parse_semver_tuple(text: str) -> Optional[Tuple[int,...|
|`_replace_versions_in_text`|fn|priv|1748-1763|def _replace_versions_in_text(text, compiled_regex, repla...|
|`_current_branch_name`|fn|priv|1767-1779|def _current_branch_name()|
|`_ref_exists`|fn|priv|1784-1793|def _ref_exists(ref_name)|
|`_local_branch_exists`|fn|priv|1798-1801|def _local_branch_exists(branch_name)|
|`_remote_branch_exists`|fn|priv|1806-1809|def _remote_branch_exists(branch_name)|
|`_ensure_release_prerequisites`|fn|priv|1813-1840|def _ensure_release_prerequisites()|
|`_bump_semver_version`|fn|priv|1846-1864|def _bump_semver_version(current_version, level)|
|`_run_release_step`|fn|priv|1871-1891|def _run_release_step(level, step_name, action)|
|`_create_release_commit_for_flow`|fn|priv|1896-1901|def _create_release_commit_for_flow(target_version)|
|`_push_branch_with_tags`|fn|priv|1907-1911|def _push_branch_with_tags(branch_name)|
|`_execute_release_flow`|fn|priv|1927-1974|def _execute_release_flow(level, changelog_args=None)|
|`_execute_backup_flow`|fn|priv|1982-1997|def _execute_backup_flow()|
|`_run_release_command`|fn|priv|2003-2018|def _run_release_command(level, changelog_args=None)|
|`_run_backup_command`|fn|priv|2023-2030|def _run_backup_command()|
|`_run_reset_with_help`|fn|priv|2036-2043|def _run_reset_with_help(base_args, extra)|
|`_reject_extra_arguments`|fn|priv|2049-2055|def _reject_extra_arguments(extra, alias)|
|`_parse_release_flags`|fn|priv|2061-2079|def _parse_release_flags(extra, alias)|
|`_prepare_commit_message`|fn|priv|2085-2095|def _prepare_commit_message(extra, alias)|
|`_normalize_conventional_description`|fn|priv|2102-2112|def _normalize_conventional_description(description: str)...|
|`_build_conventional_message`|fn|priv|2119-2134|def _build_conventional_message(kind: str, extra, alias: ...|
|`_run_conventional_commit`|fn|priv|2141-2146|def _run_conventional_commit(kind: str, alias: str, extra)|
|`_execute_commit`|fn|priv|2153-2182|def _execute_commit(message, alias, allow_amend=True)|
|`upgrade_self`|fn|pub|2186-2199|def upgrade_self()|
|`remove_self`|fn|pub|2203-2206|def remove_self()|
|`cmd_aa`|fn|pub|2211-2218|def cmd_aa(extra)|
|`cmd_ra`|fn|pub|2223-2246|def cmd_ra(extra)|
|`cmd_ar`|fn|pub|2251-2265|def cmd_ar(extra)|
|`cmd_br`|fn|pub|2270-2273|def cmd_br(extra)|
|`cmd_bd`|fn|pub|2278-2281|def cmd_bd(extra)|
|`cmd_ck`|fn|pub|2286-2289|def cmd_ck(extra)|
|`_ensure_commit_ready`|fn|priv|2294-2307|def _ensure_commit_ready(alias)|
|`cmd_cm`|fn|pub|2312-2317|def cmd_cm(extra)|
|`cmd_wip`|fn|pub|2322-2334|def cmd_wip(extra)|
|`cmd_release`|fn|pub|2339-2361|def cmd_release(extra)|
|`cmd_new`|fn|pub|2366-2369|def cmd_new(extra)|
|`cmd_refactor`|fn|pub|2374-2377|def cmd_refactor(extra)|
|`cmd_fix`|fn|pub|2382-2385|def cmd_fix(extra)|
|`cmd_change`|fn|pub|2390-2393|def cmd_change(extra)|
|`cmd_implement`|fn|pub|2398-2401|def cmd_implement(extra)|
|`cmd_docs`|fn|pub|2406-2409|def cmd_docs(extra)|
|`cmd_style`|fn|pub|2414-2417|def cmd_style(extra)|
|`cmd_revert`|fn|pub|2422-2425|def cmd_revert(extra)|
|`cmd_misc`|fn|pub|2430-2433|def cmd_misc(extra)|
|`cmd_cover`|fn|pub|2438-2441|def cmd_cover(extra)|
|`cmd_co`|fn|pub|2446-2449|def cmd_co(extra)|
|`cmd_dr`|fn|pub|2454-2461|def cmd_dr(extra)|
|`cmd_dcc`|fn|pub|2466-2469|def cmd_dcc(extra)|
|`cmd_dccc`|fn|pub|2474-2477|def cmd_dccc(extra)|
|`cmd_de`|fn|pub|2482-2485|def cmd_de(extra)|
|`cmd_di`|fn|pub|2490-2493|def cmd_di(extra)|
|`cmd_diyou`|fn|pub|2498-2501|def cmd_diyou(extra)|
|`cmd_dime`|fn|pub|2506-2509|def cmd_dime(extra)|
|`cmd_dwc`|fn|pub|2514-2517|def cmd_dwc(extra)|
|`cmd_dwcc`|fn|pub|2522-2525|def cmd_dwcc(extra)|
|`cmd_ed`|fn|pub|2530-2539|def cmd_ed(extra)|
|`cmd_fe`|fn|pub|2544-2547|def cmd_fe(extra)|
|`cmd_feall`|fn|pub|2552-2555|def cmd_feall(extra)|
|`cmd_gp`|fn|pub|2560-2563|def cmd_gp(extra)|
|`cmd_gr`|fn|pub|2568-2571|def cmd_gr(extra)|
|`OVERVIEW_COLOR_RESET`|var|pub|2573||
|`OVERVIEW_COLOR_SECTION_PURPLE`|var|pub|2575||
|`OVERVIEW_COLOR_AHEAD`|var|pub|2577||
|`OVERVIEW_COLOR_BEHIND`|var|pub|2579||
|`OVERVIEW_COLOR_LABEL`|var|pub|2581||
|`OVERVIEW_COLOR_WHITE`|var|pub|2583||
|`OVERVIEW_COLOR_WHITE_BOLD`|var|pub|2585||
|`OVERVIEW_SECTION_TEMPLATE`|var|pub|2587||
|`OVERVIEW_SUBSECTION_TEMPLATE`|var|pub|2589||
|`OVERVIEW_DISTANCE_TEMPLATE`|var|pub|2591||
|`_overview_branch_identifier`|fn|priv|2600-2603|def _overview_branch_identifier(|
|`_overview_work_prefix_color`|fn|priv|2617-2624|def _overview_work_prefix_color(worktree_state: str) -> str|
|`_overview_logical_branch_name`|fn|priv|2632-2636|def _overview_logical_branch_name(|
|`_overview_current_branch_display`|fn|priv|2655-2660|def _overview_current_branch_display(|
|`_overview_ref_is_available`|fn|priv|2682-2691|def _overview_ref_is_available(ref_name: str) -> bool|
|`_overview_ref_latest_subject`|fn|priv|2697-2706|def _overview_ref_latest_subject(ref_name: str) -> str|
|`_overview_branch_summary_lines`|fn|priv|2723-2733|def _overview_branch_summary_lines(|
|`_overview_relation_state`|fn|priv|2761-2770|def _overview_relation_state(ahead: int, behind: int) -> str|
|`_overview_worktree_state`|fn|priv|2775-2787|def _overview_worktree_state(status_lines=None) -> str|
|`_overview_distance_text`|fn|priv|2793-2801|def _overview_distance_text(is_ahead: bool, count: int) -...|
|`_overview_compare_refs`|fn|priv|2808-2848|def _overview_compare_refs(base_ref: str, target_ref: str...|
|`_overview_ascii_topology_lines`|fn|priv|2869-2880|def _overview_ascii_topology_lines(|
|`_overview_current_branch_state_lines`|fn|priv|2970-2989|def _overview_current_branch_state_lines(current_branch_d...|
|`cmd_o`|fn|pub|2995-3136|def cmd_o(extra)|
|`cmd_str`|fn|pub|3141-3170|def cmd_str(extra)|
|`cmd_lb`|fn|pub|3175-3178|def cmd_lb(extra)|
|`cmd_lg`|fn|pub|3183-3196|def cmd_lg(extra)|
|`cmd_lh`|fn|pub|3201-3204|def cmd_lh(extra)|
|`cmd_ll`|fn|pub|3209-3221|def cmd_ll(extra)|
|`cmd_lm`|fn|pub|3226-3229|def cmd_lm(extra)|
|`cmd_ls`|fn|pub|3235-3238|def cmd_ls(extra)|
|`cmd_lsi`|fn|pub|3244-3250|def cmd_lsi(extra)|
|`cmd_lsa`|fn|pub|3256-3259|def cmd_lsa(extra)|
|`cmd_lt`|fn|pub|3265-3284|def cmd_lt(extra)|
|`cmd_me`|fn|pub|3289-3292|def cmd_me(extra)|
|`cmd_pl`|fn|pub|3297-3300|def cmd_pl(extra)|
|`cmd_pt`|fn|pub|3305-3308|def cmd_pt(extra)|
|`cmd_pu`|fn|pub|3313-3316|def cmd_pu(extra)|
|`cmd_rf`|fn|pub|3321-3324|def cmd_rf(extra)|
|`cmd_rmtg`|fn|pub|3329-3339|def cmd_rmtg(extra)|
|`cmd_rmloc`|fn|pub|3344-3347|def cmd_rmloc(extra)|
|`cmd_rmstg`|fn|pub|3352-3355|def cmd_rmstg(extra)|
|`cmd_rmunt`|fn|pub|3360-3363|def cmd_rmunt(extra)|
|`cmd_rs`|fn|pub|3368-3371|def cmd_rs(extra)|
|`cmd_rssft`|fn|pub|3376-3379|def cmd_rssft(extra)|
|`cmd_rsmix`|fn|pub|3384-3387|def cmd_rsmix(extra)|
|`cmd_rshrd`|fn|pub|3392-3395|def cmd_rshrd(extra)|
|`cmd_rsmrg`|fn|pub|3400-3403|def cmd_rsmrg(extra)|
|`cmd_rskep`|fn|pub|3408-3411|def cmd_rskep(extra)|
|`cmd_st`|fn|pub|3416-3419|def cmd_st(extra)|
|`cmd_tg`|fn|pub|3424-3427|def cmd_tg(extra)|
|`cmd_unstg`|fn|pub|3432-3435|def cmd_unstg(extra)|
|`cmd_wt`|fn|pub|3440-3443|def cmd_wt(extra)|
|`cmd_wtl`|fn|pub|3448-3451|def cmd_wtl(extra)|
|`cmd_wtp`|fn|pub|3456-3459|def cmd_wtp(extra)|
|`cmd_wtr`|fn|pub|3464-3467|def cmd_wtr(extra)|
|`cmd_ver`|fn|pub|3472-3498|def cmd_ver(extra)|
|`cmd_chver`|fn|pub|3503-3575|def cmd_chver(extra)|
|`cmd_major`|fn|pub|3584-3588|def cmd_major(extra)|
|`cmd_minor`|fn|pub|3597-3601|def cmd_minor(extra)|
|`cmd_patch`|fn|pub|3610-3614|def cmd_patch(extra)|
|`cmd_backup`|fn|pub|3622-3632|def cmd_backup(extra)|
|`cmd_changelog`|fn|pub|3642-3674|def cmd_changelog(extra)|
|`COMMANDS`|var|pub|3677||
|`print_command_help`|fn|pub|3758-3764|def print_command_help(name, width=None)|
|`print_all_help`|fn|pub|3768-3802|def print_all_help()|
|`main`|fn|pub|3808-3858|def main(argv=None, *, check_updates: bool = True)|
