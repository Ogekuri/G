...
class ChangelogCommandTest(unittest.TestCase):
    ...
    def test_include_draft_flag_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = Path(tmpdir)
            with mock.patch.object(core, "is_inside_git_repo", return_value=True), mock.patch.object(
                core, "get_git_root", return_value=repo_root
    ...
    def test_generate_document_includes_all_tags_by_default(self):
        repo_root = Path("/tmp")
        release_tags = [
    ...
    def test_generate_document_uses_all_tag_ranges(self):
        tags = [
        ...
        with mock.patch.object(core, "list_tags_sorted_by_date", return_value=tags), mock.patch.object(
            ...
            core.generate_changelog_document(Path("/tmp"), include_unreleased=False)
        self.assertEqual(calls, ["v0.0.5", "v0.0.5..v0.1.0", "v0.1.0..v0.2.0"])

    def test_generate_document_includes_unreleased_with_any_tag(self):
        ...
        with mock.patch.object(core, "list_tags_sorted_by_date", return_value=tags), mock.patch.object(
            ...
            document = core.generate_changelog_document(Path("/tmp"), include_unreleased=True)
        self.assertEqual(calls, ["v0.0.5..HEAD", "v0.0.5"])
        self.assertEqual(document.count("sec"), 2)
